 ...
 * of the FacDocTemp class.  It uses the code-generated
 * FacDocTempMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacDocTemp columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_doc_temp_edit.php AND
 * fac_doc_temp_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacDocTempEditFormBase extends QForm {
	// Local instance of the FacDocTempMetaControl
	/**
	 * @var FacDocTempMetaControlGen mctFacDocTemp
	 */
	protected $mctFacDocTemp;
	...
	// Controls for FacDocTemp's Data Fields
	protected $lblCodiRegi;
	protected $txtOrdeGuia;
	protected $txtNumePara;
	protected $txtNumePeaj;
	protected $txtNumeEntr;
	protected $txtNumeDesv;
	protected $txtNumeAmar;
	protected $txtEstaOrig;
	protected $txtEstaDest;
	protected $txtPesoEnvi;
	protected $txtCodiClie;
	protected $calFechDocu;
	protected $calFechRegi;
	protected $txtCodiProd;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacDocTempMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacDocTemp = FacDocTempMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacDocTemp's data fields
		$this->lblCodiRegi = $this->mctFacDocTemp->lblCodiRegi_Create();
		$this->txtOrdeGuia = $this->mctFacDocTemp->txtOrdeGuia_Create();
		$this->txtNumePara = $this->mctFacDocTemp->txtNumePara_Create();
		$this->txtNumePeaj = $this->mctFacDocTemp->txtNumePeaj_Create();
		$this->txtNumeEntr = $this->mctFacDocTemp->txtNumeEntr_Create();
		$this->txtNumeDesv = $this->mctFacDocTemp->txtNumeDesv_Create();
		$this->txtNumeAmar = $this->mctFacDocTemp->txtNumeAmar_Create();
		$this->txtEstaOrig = $this->mctFacDocTemp->txtEstaOrig_Create();
		$this->txtEstaDest = $this->mctFacDocTemp->txtEstaDest_Create();
		$this->txtPesoEnvi = $this->mctFacDocTemp->txtPesoEnvi_Create();
		$this->txtCodiClie = $this->mctFacDocTemp->txtCodiClie_Create();
		$this->calFechDocu = $this->mctFacDocTemp->calFechDocu_Create();
		$this->calFechRegi = $this->mctFacDocTemp->calFechRegi_Create();
		$this->txtCodiProd = $this->mctFacDocTemp->txtCodiProd_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacDocTemp->FacDocTemp && !isset($_SESSION['DataFacDocTemp'])) {
            $_SESSION['DataFacDocTemp'] = serialize(array($this->mctFacDocTemp->FacDocTemp));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacDocTemp']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacDocTemp->FacDocTemp->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacDocTemp';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacDocTemp->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacDocTemp'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacDocTemp->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacDocTemp',$this->mctFacDocTemp->FacDocTemp->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacDocTemp->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacDocTemp->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_doc_temp_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacDocTemp->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacDocTempMetaControl
		$this->mctFacDocTemp->SaveFacDocTemp();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacDocTemp->TablasRelacionadasFacDocTemp();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacDocTempMetaControl
			$this->mctFacDocTemp->DeleteFacDocTemp();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacDocTemp->FacDocTemp->Id;
        $_SESSION['TablRefe'] = 'FacDocTemp';
        $_SESSION['RegiReto'] = 'fac_doc_temp_edit.php/'.$this->mctFacDocTemp->FacDocTemp->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_doc_temp_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/fac_doc_temp_edit.php');
    }

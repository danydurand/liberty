 ...
 * FacTipoProdMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacTipoProd columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_tipo_prod_edit.php AND
 * fac_tipo_prod_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacTipoProdEditFormBase extends QForm {
	// Local instance of the FacTipoProdMetaControl
	/**
	 * @var FacTipoProdMetaControlGen mctFacTipoProd
	 */
	protected $mctFacTipoProd;
	...
	// Controls for FacTipoProd's Data Fields
	protected $txtTipoProd;
	protected $txtDescTpro;
	protected $txtNombMeto;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacTipoProdMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacTipoProd = FacTipoProdMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacTipoProd's data fields
		$this->txtTipoProd = $this->mctFacTipoProd->txtTipoProd_Create();
		$this->txtDescTpro = $this->mctFacTipoProd->txtDescTpro_Create();
		$this->txtNombMeto = $this->mctFacTipoProd->txtNombMeto_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacTipoProd->FacTipoProd && !isset($_SESSION['DataFacTipoProd'])) {
            $_SESSION['DataFacTipoProd'] = serialize(array($this->mctFacTipoProd->FacTipoProd));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacTipoProd']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacTipoProd->FacTipoProd->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacTipoProd';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacTipoProd->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacTipoProd'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacTipoProd->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacTipoProd',$this->mctFacTipoProd->FacTipoProd->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacTipoProd->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacTipoProd->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_tipo_prod_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacTipoProd->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacTipoProdMetaControl
		$this->mctFacTipoProd->SaveFacTipoProd();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacTipoProd->TablasRelacionadasFacTipoProd();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtTipoProd->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacTipoProdMetaControl
			$this->mctFacTipoProd->DeleteFacTipoProd();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacTipoProd->FacTipoProd->Id;
        $_SESSION['TablRefe'] = 'FacTipoProd';
        $_SESSION['RegiReto'] = 'fac_tipo_prod_edit.php/'.$this->mctFacTipoProd->FacTipoProd->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_tipo_prod_list.php');
    }

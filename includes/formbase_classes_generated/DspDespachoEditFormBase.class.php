 ...
 * DspDespachoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a DspDespacho columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both dsp_despacho_edit.php AND
 * dsp_despacho_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class DspDespachoEditFormBase extends QForm {
	// Local instance of the DspDespachoMetaControl
	/**
	 * @var DspDespachoMetaControlGen mctDspDespacho
	 */
	protected $mctDspDespacho;
	...
	// Controls for DspDespacho's Data Fields
	protected $lblCodiDesp;
	protected $lstCodiClieObject;
	protected $lstCodiOrigObject;
	protected $lstCodiDestObject;
	protected $lstCodiOperObject;
	protected $txtNombClie;
	protected $txtDireClie;
	protected $txtTeleClie;
	protected $calFechRegi;
	protected $lstTipoRecoObject;
	protected $lstCodiCkptObject;
	protected $calFechReco;
	protected $txtHoraList;
	protected $txtHoraDesp;
	protected $txtHoraEfec;
	protected $txtHoraCier;
	protected $txtNombPers;
	protected $txtTextObse;
	protected $calFechModi;
	protected $lstCodiUsuaObject;
	protected $lstUsuaModiObject;
	protected $lstMotiNocoObject;
	protected $txtPesoEsti;
	protected $txtCantBult;
	protected $txtDireMail;
	protected $chkNotiNoco;
	protected $txtHoraRegi;
	protected $txtNombDest;
	protected $txtTeleDest;
	protected $txtDireDest;
	protected $txtContEnvi;
	protected $calFechCier;
	protected $chkPospuesta;
	protected $lstUsuaCierObject;
	protected $lstUsuaDespObject;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the DspDespachoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctDspDespacho = DspDespachoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on DspDespacho's data fields
		$this->lblCodiDesp = $this->mctDspDespacho->lblCodiDesp_Create();
		$this->lstCodiClieObject = $this->mctDspDespacho->lstCodiClieObject_Create();
		$this->lstCodiOrigObject = $this->mctDspDespacho->lstCodiOrigObject_Create();
		$this->lstCodiDestObject = $this->mctDspDespacho->lstCodiDestObject_Create();
		$this->lstCodiOperObject = $this->mctDspDespacho->lstCodiOperObject_Create();
		$this->txtNombClie = $this->mctDspDespacho->txtNombClie_Create();
		$this->txtDireClie = $this->mctDspDespacho->txtDireClie_Create();
		$this->txtTeleClie = $this->mctDspDespacho->txtTeleClie_Create();
		$this->calFechRegi = $this->mctDspDespacho->calFechRegi_Create();
		$this->lstTipoRecoObject = $this->mctDspDespacho->lstTipoRecoObject_Create();
		$this->lstCodiCkptObject = $this->mctDspDespacho->lstCodiCkptObject_Create();
		$this->calFechReco = $this->mctDspDespacho->calFechReco_Create();
		$this->txtHoraList = $this->mctDspDespacho->txtHoraList_Create();
		$this->txtHoraDesp = $this->mctDspDespacho->txtHoraDesp_Create();
		$this->txtHoraEfec = $this->mctDspDespacho->txtHoraEfec_Create();
		$this->txtHoraCier = $this->mctDspDespacho->txtHoraCier_Create();
		$this->txtNombPers = $this->mctDspDespacho->txtNombPers_Create();
		$this->txtTextObse = $this->mctDspDespacho->txtTextObse_Create();
		$this->calFechModi = $this->mctDspDespacho->calFechModi_Create();
		$this->lstCodiUsuaObject = $this->mctDspDespacho->lstCodiUsuaObject_Create();
		$this->lstUsuaModiObject = $this->mctDspDespacho->lstUsuaModiObject_Create();
		$this->lstMotiNocoObject = $this->mctDspDespacho->lstMotiNocoObject_Create();
		$this->txtPesoEsti = $this->mctDspDespacho->txtPesoEsti_Create();
		$this->txtCantBult = $this->mctDspDespacho->txtCantBult_Create();
		$this->txtDireMail = $this->mctDspDespacho->txtDireMail_Create();
		$this->chkNotiNoco = $this->mctDspDespacho->chkNotiNoco_Create();
		$this->txtHoraRegi = $this->mctDspDespacho->txtHoraRegi_Create();
		$this->txtNombDest = $this->mctDspDespacho->txtNombDest_Create();
		$this->txtTeleDest = $this->mctDspDespacho->txtTeleDest_Create();
		$this->txtDireDest = $this->mctDspDespacho->txtDireDest_Create();
		$this->txtContEnvi = $this->mctDspDespacho->txtContEnvi_Create();
		$this->calFechCier = $this->mctDspDespacho->calFechCier_Create();
		$this->chkPospuesta = $this->mctDspDespacho->chkPospuesta_Create();
		$this->lstUsuaCierObject = $this->mctDspDespacho->lstUsuaCierObject_Create();
		$this->lstUsuaDespObject = $this->mctDspDespacho->lstUsuaDespObject_Create();
		$this->calFechDesp = $this->mctDspDespacho->calFechDesp_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctDspDespacho->DspDespacho && !isset($_SESSION['DataDspDespacho'])) {
            $_SESSION['DataDspDespacho'] = serialize(array($this->mctDspDespacho->DspDespacho));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataDspDespacho']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctDspDespacho->DspDespacho->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'DspDespacho';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctDspDespacho->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('DspDespacho'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctDspDespacho->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('DspDespacho',$this->mctDspDespacho->DspDespacho->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctDspDespacho->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctDspDespacho->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/dsp_despacho_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctDspDespacho->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the DspDespachoMetaControl
		$this->mctDspDespacho->SaveDspDespacho();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctDspDespacho->TablasRelacionadasDspDespacho();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lblCodiDesp->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the DspDespachoMetaControl
			$this->mctDspDespacho->DeleteDspDespacho();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctDspDespacho->DspDespacho->Id;
        $_SESSION['TablRefe'] = 'DspDespacho';
        $_SESSION['RegiReto'] = 'dsp_despacho_edit.php/'.$this->mctDspDespacho->DspDespacho->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/dsp_despacho_list.php');
    }

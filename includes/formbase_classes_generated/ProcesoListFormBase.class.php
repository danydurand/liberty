<?php
	...
	abstract class ProcesoListFormBase extends QForm {
		...
		protected function Form_Create() {
			...
			$this->dtgProcesos->FontSize = 13;
			$this->dtgProcesos->ShowFilter = false;

			// Add Pagination (if desired)
			$this->dtgProcesos->Paginator = new QPaginator($this->dtgProcesos);
			$this->dtgProcesos->ItemsPerPage = __FORM_DRAFTS_FORM_LIST_ITEMS_PER_PAGE__;

			// Higlight the datagrid rows when mousing over them
			$this->dtgProcesos->AddRowAction(new QMouseOverEvent(), new QCssClassAction('selectedStyle'));
			$this->dtgProcesos->AddRowAction(new QMouseOutEvent(), new QCssClassAction());

			// Add a click handler for the rows.
			// We can use $_CONTROL->CurrentRowIndex to pass the row index to dtgPersonsRow_Click()
			// or $_ITEM->Id to pass the object's id, or any other data grid variable
			$this->dtgProcesos->RowActionParameterHtml = '<?= $_ITEM->Id ?>';
			$this->dtgProcesos->AddRowAction(new QClickEvent(), new QAjaxAction('dtgProcesosRow_Click'));
			...
			$this->dtgProcesos->MetaAddColumn('Id');
			$this->dtgProcesos->MetaAddColumn('Usuario');
			$this->dtgProcesos->MetaAddColumn('Programa');
			$this->dtgProcesos->MetaAddColumn('Fecha');
			$this->dtgProcesos->MetaAddColumn('Hora');
			$this->dtgProcesos->MetaAddColumn('Accion');
		...
		protected function lblTituForm_Create() {
			...
			$this->lblTituForm->Text = 'Procesos';
		}
        ...
        protected function btnExpoExce_Create() {
            $this->btnExpoExce = new QDataGridExporterButton($this, $this->dtgProcesos);
        ...
        protected function btnFiltAvan_Click() {
            $this->dtgProcesos->ShowFilter = !$this->dtgProcesos->ShowFilter;
        }

		public function dtgProcesosRow_Click($strFormId, $strControlId, $strParameter) {
		  $intId = intval($strParameter);
		  QApplication::Redirect("proceso_edit.php/$intId");
		}

 ...
 * MasCartaDevoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a MasCartaDevo columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both mas_carta_devo_edit.php AND
 * mas_carta_devo_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class MasCartaDevoEditFormBase extends QForm {
	// Local instance of the MasCartaDevoMetaControl
	/**
	 * @var MasCartaDevoMetaControlGen mctMasCartaDevo
	 */
	protected $mctMasCartaDevo;
	...
	// Controls for MasCartaDevo's Data Fields
	protected $lblNumeCart;
	protected $lstCodiClieObject;
	protected $lstCodiCateObject;
	protected $calFechCart;
	protected $txtCantEntr;
	protected $txtCantInci;
	protected $txtNumePrec;
	protected $lstStatCartObject;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the MasCartaDevoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctMasCartaDevo = MasCartaDevoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on MasCartaDevo's data fields
		$this->lblNumeCart = $this->mctMasCartaDevo->lblNumeCart_Create();
		$this->lstCodiClieObject = $this->mctMasCartaDevo->lstCodiClieObject_Create();
		$this->lstCodiCateObject = $this->mctMasCartaDevo->lstCodiCateObject_Create();
		$this->calFechCart = $this->mctMasCartaDevo->calFechCart_Create();
		$this->txtCantEntr = $this->mctMasCartaDevo->txtCantEntr_Create();
		$this->txtCantInci = $this->mctMasCartaDevo->txtCantInci_Create();
		$this->txtNumePrec = $this->mctMasCartaDevo->txtNumePrec_Create();
		$this->lstStatCartObject = $this->mctMasCartaDevo->lstStatCartObject_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctMasCartaDevo->MasCartaDevo && !isset($_SESSION['DataMasCartaDevo'])) {
            $_SESSION['DataMasCartaDevo'] = serialize(array($this->mctMasCartaDevo->MasCartaDevo));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataMasCartaDevo']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctMasCartaDevo->MasCartaDevo->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'MasCartaDevo';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctMasCartaDevo->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('MasCartaDevo'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctMasCartaDevo->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('MasCartaDevo',$this->mctMasCartaDevo->MasCartaDevo->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctMasCartaDevo->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctMasCartaDevo->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/mas_carta_devo_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctMasCartaDevo->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the MasCartaDevoMetaControl
		$this->mctMasCartaDevo->SaveMasCartaDevo();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctMasCartaDevo->TablasRelacionadasMasCartaDevo();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lblNumeCart->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the MasCartaDevoMetaControl
			$this->mctMasCartaDevo->DeleteMasCartaDevo();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctMasCartaDevo->MasCartaDevo->Id;
        $_SESSION['TablRefe'] = 'MasCartaDevo';
        $_SESSION['RegiReto'] = 'mas_carta_devo_edit.php/'.$this->mctMasCartaDevo->MasCartaDevo->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/mas_carta_devo_list.php');
    }

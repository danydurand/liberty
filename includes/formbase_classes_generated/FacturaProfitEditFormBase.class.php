 ...
 * of the FacturaProfit class.  It uses the code-generated
 * FacturaProfitMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacturaProfit columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both factura_profit_edit.php AND
 * factura_profit_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacturaProfitEditFormBase extends QForm {
	// Local instance of the FacturaProfitMetaControl
	/**
	 * @var FacturaProfitMetaControlGen mctFacturaProfit
	 */
	protected $mctFacturaProfit;
	...
	// Controls for FacturaProfit's Data Fields
	protected $lblId;
	protected $txtNroFactura;
	protected $calFecha;
	protected $calFechaCarga;
	protected $txtReceptoriaId;
	protected $txtStatusId;
	protected $txtMonto;
	protected $txtMontoGuias;
	protected $txtLote;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacturaProfitMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacturaProfit = FacturaProfitMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacturaProfit's data fields
		$this->lblId = $this->mctFacturaProfit->lblId_Create();
		$this->txtNroFactura = $this->mctFacturaProfit->txtNroFactura_Create();
		$this->calFecha = $this->mctFacturaProfit->calFecha_Create();
		$this->calFechaCarga = $this->mctFacturaProfit->calFechaCarga_Create();
		$this->txtReceptoriaId = $this->mctFacturaProfit->txtReceptoriaId_Create();
		$this->txtStatusId = $this->mctFacturaProfit->txtStatusId_Create();
		$this->txtMonto = $this->mctFacturaProfit->txtMonto_Create();
		$this->txtMontoGuias = $this->mctFacturaProfit->txtMontoGuias_Create();
		$this->txtLote = $this->mctFacturaProfit->txtLote_Create();
		$this->txtUsuarioId = $this->mctFacturaProfit->txtUsuarioId_Create();
		$this->txtMotivoAnulacion = $this->mctFacturaProfit->txtMotivoAnulacion_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacturaProfit->FacturaProfit && !isset($_SESSION['DataFacturaProfit'])) {
            $_SESSION['DataFacturaProfit'] = serialize(array($this->mctFacturaProfit->FacturaProfit));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacturaProfit']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacturaProfit->FacturaProfit->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacturaProfit';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacturaProfit->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacturaProfit'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacturaProfit->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacturaProfit',$this->mctFacturaProfit->FacturaProfit->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacturaProfit->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacturaProfit->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objFacturaProfit = FacturaProfit::LoadByNroFactura($this->txtNroFactura->Text)) && ($objFacturaProfit->Id != $this->mctFacturaProfit->FacturaProfit->Id )){
				$blnToReturn = false;
				$this->txtNroFactura->Warning = QApplication::Translate("Already in Use");
			}
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/factura_profit_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacturaProfit->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacturaProfitMetaControl
		$this->mctFacturaProfit->SaveFacturaProfit();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacturaProfit->TablasRelacionadasFacturaProfit();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacturaProfitMetaControl
			$this->mctFacturaProfit->DeleteFacturaProfit();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacturaProfit->FacturaProfit->Id;
        $_SESSION['TablRefe'] = 'FacturaProfit';
        $_SESSION['RegiReto'] = 'factura_profit_edit.php/'.$this->mctFacturaProfit->FacturaProfit->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/factura_profit_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/factura_profit_edit.php');
    }

 ...
 * ItemNotaCreditoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ItemNotaCredito columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both item_nota_credito_edit.php AND
 * item_nota_credito_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ItemNotaCreditoEditFormBase extends QForm {
	// Local instance of the ItemNotaCreditoMetaControl
	/**
	 * @var ItemNotaCreditoMetaControlGen mctItemNotaCredito
	 */
	protected $mctItemNotaCredito;
	...
	// Controls for ItemNotaCredito's Data Fields
	protected $lblId;
	protected $lstNotaCredito;
	protected $lstGuia;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ItemNotaCreditoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctItemNotaCredito = ItemNotaCreditoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ItemNotaCredito's data fields
		$this->lblId = $this->mctItemNotaCredito->lblId_Create();
		$this->lstNotaCredito = $this->mctItemNotaCredito->lstNotaCredito_Create();
		$this->lstGuia = $this->mctItemNotaCredito->lstGuia_Create();
		$this->txtMontoBase = $this->mctItemNotaCredito->txtMontoBase_Create();
		$this->txtPorcentajeDscto = $this->mctItemNotaCredito->txtPorcentajeDscto_Create();
		$this->txtMontoDscto = $this->mctItemNotaCredito->txtMontoDscto_Create();
		$this->txtMontoFranqueo = $this->mctItemNotaCredito->txtMontoFranqueo_Create();
		$this->txtPorcentajeIva = $this->mctItemNotaCredito->txtPorcentajeIva_Create();
		$this->txtMontoIva = $this->mctItemNotaCredito->txtMontoIva_Create();
		$this->txtMontoSeguro = $this->mctItemNotaCredito->txtMontoSeguro_Create();
		$this->txtMontoOtros = $this->mctItemNotaCredito->txtMontoOtros_Create();
		$this->txtMontoTotal = $this->mctItemNotaCredito->txtMontoTotal_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctItemNotaCredito->ItemNotaCredito && !isset($_SESSION['DataItemNotaCredito'])) {
            $_SESSION['DataItemNotaCredito'] = serialize(array($this->mctItemNotaCredito->ItemNotaCredito));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataItemNotaCredito']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctItemNotaCredito->ItemNotaCredito->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ItemNotaCredito';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctItemNotaCredito->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ItemNotaCredito'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctItemNotaCredito->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ItemNotaCredito',$this->mctItemNotaCredito->ItemNotaCredito->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctItemNotaCredito->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctItemNotaCredito->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/item_nota_credito_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctItemNotaCredito->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ItemNotaCreditoMetaControl
		$this->mctItemNotaCredito->SaveItemNotaCredito();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctItemNotaCredito->TablasRelacionadasItemNotaCredito();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ItemNotaCreditoMetaControl
			$this->mctItemNotaCredito->DeleteItemNotaCredito();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctItemNotaCredito->ItemNotaCredito->Id;
        $_SESSION['TablRefe'] = 'ItemNotaCredito';
        $_SESSION['RegiReto'] = 'item_nota_credito_edit.php/'.$this->mctItemNotaCredito->ItemNotaCredito->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/item_nota_credito_list.php');
    }

 ...
 * of the SreGuiaCkpt class.  It uses the code-generated
 * SreGuiaCkptMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a SreGuiaCkpt columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both sre_guia_ckpt_edit.php AND
 * sre_guia_ckpt_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class SreGuiaCkptEditFormBase extends QForm {
	// Local instance of the SreGuiaCkptMetaControl
	/**
	 * @var SreGuiaCkptMetaControlGen mctSreGuiaCkpt
	 */
	protected $mctSreGuiaCkpt;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the SreGuiaCkptMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctSreGuiaCkpt = SreGuiaCkptMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on SreGuiaCkpt's data fields
		$this->lstNumeGuiaObject = $this->mctSreGuiaCkpt->lstNumeGuiaObject_Create();
		$this->lstCodiEstaObject = $this->mctSreGuiaCkpt->lstCodiEstaObject_Create();
		$this->lstCodiCkptObject = $this->mctSreGuiaCkpt->lstCodiCkptObject_Create();
		$this->calFechCkpt = $this->mctSreGuiaCkpt->calFechCkpt_Create();
		$this->txtHoraCkpt = $this->mctSreGuiaCkpt->txtHoraCkpt_Create();
		$this->txtTextObse = $this->mctSreGuiaCkpt->txtTextObse_Create();
		$this->txtCodiRuta = $this->mctSreGuiaCkpt->txtCodiRuta_Create();
		$this->lstCodiUsuaObject = $this->mctSreGuiaCkpt->lstCodiUsuaObject_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctSreGuiaCkpt->SreGuiaCkpt && !isset($_SESSION['DataSreGuiaCkpt'])) {
            $_SESSION['DataSreGuiaCkpt'] = serialize(array($this->mctSreGuiaCkpt->SreGuiaCkpt));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataSreGuiaCkpt']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctSreGuiaCkpt->SreGuiaCkpt->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'SreGuiaCkpt';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctSreGuiaCkpt->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('SreGuiaCkpt'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctSreGuiaCkpt->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('SreGuiaCkpt',$this->mctSreGuiaCkpt->SreGuiaCkpt->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctSreGuiaCkpt->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctSreGuiaCkpt->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/sre_guia_ckpt_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctSreGuiaCkpt->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the SreGuiaCkptMetaControl
		$this->mctSreGuiaCkpt->SaveSreGuiaCkpt();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctSreGuiaCkpt->TablasRelacionadasSreGuiaCkpt();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lstNumeGuiaObject->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->lstCodiEstaObject->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->lstCodiCkptObject->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->calFechCkpt->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->txtHoraCkpt->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the SreGuiaCkptMetaControl
			$this->mctSreGuiaCkpt->DeleteSreGuiaCkpt();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctSreGuiaCkpt->SreGuiaCkpt->Id;
        $_SESSION['TablRefe'] = 'SreGuiaCkpt';
        $_SESSION['RegiReto'] = 'sre_guia_ckpt_edit.php/'.$this->mctSreGuiaCkpt->SreGuiaCkpt->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/sre_guia_ckpt_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/sre_guia_ckpt_edit.php');
    }

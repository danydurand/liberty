 ...
 * of the ClienteInt class.  It uses the code-generated
 * ClienteIntMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ClienteInt columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both cliente_int_edit.php AND
 * cliente_int_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ClienteIntEditFormBase extends QForm {
	// Local instance of the ClienteIntMetaControl
	/**
	 * @var ClienteIntMetaControlGen mctClienteInt
	 */
	protected $mctClienteInt;
	...
	// Controls for ClienteInt's Data Fields
	protected $txtCodCliente;
	protected $txtClienteRs;
	protected $txtCidrif;
	protected $txtCodContacto;
	protected $txtNombreContacto;
	protected $txtIata;
	protected $txtDirContacto;
	protected $txtTlfContacto;
	protected $txtPaisContacto;
	protected $txtEstadoContacto;
	protected $txtCiudadContacto;
	protected $txtEmailContacto;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ClienteIntMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctClienteInt = ClienteIntMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ClienteInt's data fields
		$this->txtCodCliente = $this->mctClienteInt->txtCodCliente_Create();
		$this->txtClienteRs = $this->mctClienteInt->txtClienteRs_Create();
		$this->txtCidrif = $this->mctClienteInt->txtCidrif_Create();
		$this->txtCodContacto = $this->mctClienteInt->txtCodContacto_Create();
		$this->txtNombreContacto = $this->mctClienteInt->txtNombreContacto_Create();
		$this->txtIata = $this->mctClienteInt->txtIata_Create();
		$this->txtDirContacto = $this->mctClienteInt->txtDirContacto_Create();
		$this->txtTlfContacto = $this->mctClienteInt->txtTlfContacto_Create();
		$this->txtPaisContacto = $this->mctClienteInt->txtPaisContacto_Create();
		$this->txtEstadoContacto = $this->mctClienteInt->txtEstadoContacto_Create();
		$this->txtCiudadContacto = $this->mctClienteInt->txtCiudadContacto_Create();
		$this->txtEmailContacto = $this->mctClienteInt->txtEmailContacto_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctClienteInt->ClienteInt && !isset($_SESSION['DataClienteInt'])) {
            $_SESSION['DataClienteInt'] = serialize(array($this->mctClienteInt->ClienteInt));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataClienteInt']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctClienteInt->ClienteInt->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ClienteInt';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctClienteInt->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ClienteInt'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctClienteInt->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ClienteInt',$this->mctClienteInt->ClienteInt->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctClienteInt->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctClienteInt->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/cliente_int_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctClienteInt->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ClienteIntMetaControl
		$this->mctClienteInt->SaveClienteInt();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctClienteInt->TablasRelacionadasClienteInt();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtCodContacto->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ClienteIntMetaControl
			$this->mctClienteInt->DeleteClienteInt();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctClienteInt->ClienteInt->Id;
        $_SESSION['TablRefe'] = 'ClienteInt';
        $_SESSION['RegiReto'] = 'cliente_int_edit.php/'.$this->mctClienteInt->ClienteInt->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/cliente_int_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/cliente_int_edit.php');
    }

 ...
 * of the TasaCambio class.  It uses the code-generated
 * TasaCambioMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a TasaCambio columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both tasa_cambio_edit.php AND
 * tasa_cambio_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class TasaCambioEditFormBase extends QForm {
	// Local instance of the TasaCambioMetaControl
	/**
	 * @var TasaCambioMetaControlGen mctTasaCambio
	 */
	protected $mctTasaCambio;
	...
	// Controls for TasaCambio's Data Fields
	protected $lblId;
	protected $lstDivisa;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the TasaCambioMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctTasaCambio = TasaCambioMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on TasaCambio's data fields
		$this->lblId = $this->mctTasaCambio->lblId_Create();
		$this->lstDivisa = $this->mctTasaCambio->lstDivisa_Create();
		$this->calFechaVigencia = $this->mctTasaCambio->calFechaVigencia_Create();
		$this->txtMonto = $this->mctTasaCambio->txtMonto_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctTasaCambio->TasaCambio && !isset($_SESSION['DataTasaCambio'])) {
            $_SESSION['DataTasaCambio'] = serialize(array($this->mctTasaCambio->TasaCambio));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataTasaCambio']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctTasaCambio->TasaCambio->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'TasaCambio';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctTasaCambio->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('TasaCambio'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctTasaCambio->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('TasaCambio',$this->mctTasaCambio->TasaCambio->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctTasaCambio->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctTasaCambio->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objTasaCambio = TasaCambio::LoadByDivisaIdFechaVigencia($this->lstDivisa->SelectedValue,$this->calFechaVigencia->DateTime)) && ($objTasaCambio->Id != $this->mctTasaCambio->TasaCambio->Id )){
				$blnToReturn = false;
				$this->lstDivisa->Warning = QApplication::Translate("Already in Use");
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/tasa_cambio_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctTasaCambio->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the TasaCambioMetaControl
		$this->mctTasaCambio->SaveTasaCambio();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctTasaCambio->TablasRelacionadasTasaCambio();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the TasaCambioMetaControl
			$this->mctTasaCambio->DeleteTasaCambio();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctTasaCambio->TasaCambio->Id;
        $_SESSION['TablRefe'] = 'TasaCambio';
        $_SESSION['RegiReto'] = 'tasa_cambio_edit.php/'.$this->mctTasaCambio->TasaCambio->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/tasa_cambio_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/tasa_cambio_edit.php');
    }

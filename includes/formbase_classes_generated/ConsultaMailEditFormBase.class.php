 ...
 * of the ConsultaMail class.  It uses the code-generated
 * ConsultaMailMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ConsultaMail columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both consulta_mail_edit.php AND
 * consulta_mail_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ConsultaMailEditFormBase extends QForm {
	// Local instance of the ConsultaMailMetaControl
	/**
	 * @var ConsultaMailMetaControlGen mctConsultaMail
	 */
	protected $mctConsultaMail;
	...
	// Controls for ConsultaMail's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ConsultaMailMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctConsultaMail = ConsultaMailMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ConsultaMail's data fields
		$this->lblId = $this->mctConsultaMail->lblId_Create();
		$this->txtClienteId = $this->mctConsultaMail->txtClienteId_Create();
		$this->txtCedula = $this->mctConsultaMail->txtCedula_Create();
		$this->txtGuiaId = $this->mctConsultaMail->txtGuiaId_Create();
		$this->txtFechaEmail = $this->mctConsultaMail->txtFechaEmail_Create();
		$this->calFechaRegistro = $this->mctConsultaMail->calFechaRegistro_Create();
		$this->txtHoraRegistro = $this->mctConsultaMail->txtHoraRegistro_Create();
		$this->txtEmail = $this->mctConsultaMail->txtEmail_Create();
		$this->txtProcesado = $this->mctConsultaMail->txtProcesado_Create();
		$this->calFechaProcesado = $this->mctConsultaMail->calFechaProcesado_Create();
		$this->txtHoraProcesado = $this->mctConsultaMail->txtHoraProcesado_Create();
		$this->txtStatus = $this->mctConsultaMail->txtStatus_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctConsultaMail->ConsultaMail && !isset($_SESSION['DataConsultaMail'])) {
            $_SESSION['DataConsultaMail'] = serialize(array($this->mctConsultaMail->ConsultaMail));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataConsultaMail']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctConsultaMail->ConsultaMail->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ConsultaMail';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctConsultaMail->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ConsultaMail'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctConsultaMail->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ConsultaMail',$this->mctConsultaMail->ConsultaMail->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctConsultaMail->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctConsultaMail->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/consulta_mail_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctConsultaMail->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ConsultaMailMetaControl
		$this->mctConsultaMail->SaveConsultaMail();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctConsultaMail->TablasRelacionadasConsultaMail();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ConsultaMailMetaControl
			$this->mctConsultaMail->DeleteConsultaMail();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctConsultaMail->ConsultaMail->Id;
        $_SESSION['TablRefe'] = 'ConsultaMail';
        $_SESSION['RegiReto'] = 'consulta_mail_edit.php/'.$this->mctConsultaMail->ConsultaMail->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/consulta_mail_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/consulta_mail_edit.php');
    }

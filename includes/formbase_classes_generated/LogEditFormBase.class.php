 ...
 * LogMetaControl class, which has meta-methods to help with
 ...
 * permanent changes, it is STRONGLY RECOMMENDED to move both log_edit.php AND
 * log_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class LogEditFormBase extends QForm {
	// Local instance of the LogMetaControl
	/**
	 * @var LogMetaControlGen mctLog
	 */
	protected $mctLog;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the LogMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctLog = LogMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on Log's data fields
		$this->lblId = $this->mctLog->lblId_Create();
		$this->calFecha = $this->mctLog->calFecha_Create();
		$this->calHora = $this->mctLog->calHora_Create();
		$this->txtUsuario = $this->mctLog->txtUsuario_Create();
		$this->txtTabla = $this->mctLog->txtTabla_Create();
		$this->txtRef = $this->mctLog->txtRef_Create();
		$this->txtNombre = $this->mctLog->txtNombre_Create();
		$this->txtDescripcion = $this->mctLog->txtDescripcion_Create();
		$this->txtSistema = $this->mctLog->txtSistema_Create();
		$this->chkDelicado = $this->mctLog->chkDelicado_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctLog->Log && !isset($_SESSION['DataLog'])) {
            $_SESSION['DataLog'] = serialize(array($this->mctLog->Log));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataLog']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctLog->Log->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctLog->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Log'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctLog->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Log',$this->mctLog->Log->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctLog->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctLog->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/log_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the LogMetaControl
		$this->mctLog->SaveLog();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctLog->TablasRelacionadasLog();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the LogMetaControl
    ...
    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctLog->Log->Id;
        $_SESSION['TablRefe'] = 'Log';
        $_SESSION['RegiReto'] = 'log_edit.php/'.$this->mctLog->Log->Id;
    ...
    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/log_edit.php');
    }

 ...
 * FacProfitMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacProfit columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_profit_edit.php AND
 * fac_profit_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacProfitEditFormBase extends QForm {
	// Local instance of the FacProfitMetaControl
	/**
	 * @var FacProfitMetaControlGen mctFacProfit
	 */
	protected $mctFacProfit;
	...
	// Controls for FacProfit's Data Fields
	protected $txtNumeroFactura;
	protected $txtCondicionPago;
	protected $calFechaEmision;
	protected $calFechaVencimiento;
	protected $txtCodigoCliente;
	protected $txtCodigoVendedor;
	protected $txtCodigoTransporte;
	protected $txtDescripcionFactura;
	protected $txtNumeroControl;
	protected $txtMoneda;
	protected $txtCodigoArticulo;
	protected $txtCodigoAlmacen;
	protected $txtUnidad;
	protected $txtCantidad;
	protected $txtPorcentajeDscto;
	protected $txtPrecioArticulo;
	protected $txtImpuestoAsignado;
	protected $txtDsctoGlobal;
	protected $txtRecargo;
	protected $txtSucursal;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacProfitMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacProfit = FacProfitMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacProfit's data fields
		$this->txtNumeroFactura = $this->mctFacProfit->txtNumeroFactura_Create();
		$this->txtCondicionPago = $this->mctFacProfit->txtCondicionPago_Create();
		$this->calFechaEmision = $this->mctFacProfit->calFechaEmision_Create();
		$this->calFechaVencimiento = $this->mctFacProfit->calFechaVencimiento_Create();
		$this->txtCodigoCliente = $this->mctFacProfit->txtCodigoCliente_Create();
		$this->txtCodigoVendedor = $this->mctFacProfit->txtCodigoVendedor_Create();
		$this->txtCodigoTransporte = $this->mctFacProfit->txtCodigoTransporte_Create();
		$this->txtDescripcionFactura = $this->mctFacProfit->txtDescripcionFactura_Create();
		$this->txtNumeroControl = $this->mctFacProfit->txtNumeroControl_Create();
		$this->txtMoneda = $this->mctFacProfit->txtMoneda_Create();
		$this->txtCodigoArticulo = $this->mctFacProfit->txtCodigoArticulo_Create();
		$this->txtCodigoAlmacen = $this->mctFacProfit->txtCodigoAlmacen_Create();
		$this->txtUnidad = $this->mctFacProfit->txtUnidad_Create();
		$this->txtCantidad = $this->mctFacProfit->txtCantidad_Create();
		$this->txtPorcentajeDscto = $this->mctFacProfit->txtPorcentajeDscto_Create();
		$this->txtPrecioArticulo = $this->mctFacProfit->txtPrecioArticulo_Create();
		$this->txtImpuestoAsignado = $this->mctFacProfit->txtImpuestoAsignado_Create();
		$this->txtDsctoGlobal = $this->mctFacProfit->txtDsctoGlobal_Create();
		$this->txtRecargo = $this->mctFacProfit->txtRecargo_Create();
		$this->txtSucursal = $this->mctFacProfit->txtSucursal_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacProfit->FacProfit && !isset($_SESSION['DataFacProfit'])) {
            $_SESSION['DataFacProfit'] = serialize(array($this->mctFacProfit->FacProfit));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacProfit']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacProfit->FacProfit->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacProfit';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacProfit->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacProfit'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacProfit->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacProfit',$this->mctFacProfit->FacProfit->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacProfit->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacProfit->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_profit_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacProfitMetaControl
		$this->mctFacProfit->SaveFacProfit();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacProfit->TablasRelacionadasFacProfit();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtNumeroFactura->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacProfitMetaControl
			$this->mctFacProfit->DeleteFacProfit();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacProfit->FacProfit->Id;
        $_SESSION['TablRefe'] = 'FacProfit';
        $_SESSION['RegiReto'] = 'fac_profit_edit.php/'.$this->mctFacProfit->FacProfit->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_profit_list.php');
    }

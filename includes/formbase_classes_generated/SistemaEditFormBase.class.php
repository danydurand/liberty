 ...
 * of the Sistema class.  It uses the code-generated
 * SistemaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a Sistema columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both sistema_edit.php AND
 * sistema_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class SistemaEditFormBase extends QForm {
	// Local instance of the SistemaMetaControl
	/**
	 * @var SistemaMetaControlGen mctSistema
	 */
	protected $mctSistema;
	...
	// Controls for Sistema's Data Fields
	protected $txtCodiSist;
	protected $txtDescSist;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the SistemaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctSistema = SistemaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on Sistema's data fields
		$this->txtCodiSist = $this->mctSistema->txtCodiSist_Create();
		$this->txtDescSist = $this->mctSistema->txtDescSist_Create();
		$this->lstCodiStatObject = $this->mctSistema->lstCodiStatObject_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctSistema->Sistema && !isset($_SESSION['DataSistema'])) {
            $_SESSION['DataSistema'] = serialize(array($this->mctSistema->Sistema));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataSistema']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctSistema->Sistema->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctSistema->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Sistema'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctSistema->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Sistema',$this->mctSistema->Sistema->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctSistema->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctSistema->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/sistema_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the SistemaMetaControl
		$this->mctSistema->SaveSistema();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctSistema->TablasRelacionadasSistema();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtCodiSist->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the SistemaMetaControl
			$this->mctSistema->DeleteSistema();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctSistema->Sistema->Id;
        $_SESSION['TablRefe'] = 'Sistema';
        $_SESSION['RegiReto'] = 'sistema_edit.php/'.$this->mctSistema->Sistema->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/sistema_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/sistema_edit.php');
    }

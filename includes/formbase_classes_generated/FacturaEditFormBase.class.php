 ...
 * FacturaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a Factura columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both factura_edit.php AND
 * factura_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacturaEditFormBase extends QForm {
	// Local instance of the FacturaMetaControl
	/**
	 * @var FacturaMetaControlGen mctFactura
	 */
	protected $mctFactura;
	...
	// Controls for Factura's Data Fields
	protected $lblId;
	...
	protected $lstUsuarioCreacionObject;
	protected $lstStatus;
	protected $txtControlId;
	protected $lstUsuarioAnulacionObject;
	protected $txtMotivoAnulacion;
	protected $calFechaAnulacion;
	protected $calFechaTrans;
	protected $txtHoraTrans;
	protected $lstTipoDocumento;
	protected $txtCedulaRif;
	protected $txtNumero;
	protected $txtNumeroDeGuias;
	protected $txtDsctoPorVolumen;
	protected $txtMontoDsctoVolumen;
	protected $txtPesoTotal;
	protected $txtDsctoPorPeso;
	protected $txtMontoDsctoPeso;
	protected $txtTicketFiscal;
	protected $txtPersonaAutorizada;
	protected $lstImpresaObject;
	protected $lstSistema;
	protected $txtDireccionEntrega;
	protected $txtTipoImpresora;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacturaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFactura = FacturaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on Factura's data fields
		$this->lblId = $this->mctFactura->lblId_Create();
		$this->lstCodiClieObject = $this->mctFactura->lstCodiClieObject_Create();
		$this->lstCliente = $this->mctFactura->lstCliente_Create();
		$this->txtRazonSocial = $this->mctFactura->txtRazonSocial_Create();
		$this->txtDireccion = $this->mctFactura->txtDireccion_Create();
		$this->txtTelefono = $this->mctFactura->txtTelefono_Create();
		$this->calFecha = $this->mctFactura->calFecha_Create();
		$this->txtSerie = $this->mctFactura->txtSerie_Create();
		$this->txtControlSeniat = $this->mctFactura->txtControlSeniat_Create();
		$this->txtMontoBase = $this->mctFactura->txtMontoBase_Create();
		$this->txtMontoFranqueo = $this->mctFactura->txtMontoFranqueo_Create();
		$this->txtPorcentajeIva = $this->mctFactura->txtPorcentajeIva_Create();
		$this->txtMontoIva = $this->mctFactura->txtMontoIva_Create();
		$this->txtMontoSeguro = $this->mctFactura->txtMontoSeguro_Create();
		$this->txtMontoOtros = $this->mctFactura->txtMontoOtros_Create();
		$this->txtMontoAduana = $this->mctFactura->txtMontoAduana_Create();
		$this->txtMontoTotal = $this->mctFactura->txtMontoTotal_Create();
		$this->lstCaja = $this->mctFactura->lstCaja_Create();
		$this->lstSucursal = $this->mctFactura->lstSucursal_Create();
		$this->lstUsuarioCreacionObject = $this->mctFactura->lstUsuarioCreacionObject_Create();
		$this->lstStatus = $this->mctFactura->lstStatus_Create();
		$this->txtControlId = $this->mctFactura->txtControlId_Create();
		$this->lstUsuarioAnulacionObject = $this->mctFactura->lstUsuarioAnulacionObject_Create();
		$this->txtMotivoAnulacion = $this->mctFactura->txtMotivoAnulacion_Create();
		$this->calFechaAnulacion = $this->mctFactura->calFechaAnulacion_Create();
		$this->calFechaTrans = $this->mctFactura->calFechaTrans_Create();
		$this->txtHoraTrans = $this->mctFactura->txtHoraTrans_Create();
		$this->lstTipoDocumento = $this->mctFactura->lstTipoDocumento_Create();
		$this->txtCedulaRif = $this->mctFactura->txtCedulaRif_Create();
		$this->txtNumero = $this->mctFactura->txtNumero_Create();
		$this->txtNumeroDeGuias = $this->mctFactura->txtNumeroDeGuias_Create();
		$this->txtDsctoPorVolumen = $this->mctFactura->txtDsctoPorVolumen_Create();
		$this->txtMontoDsctoVolumen = $this->mctFactura->txtMontoDsctoVolumen_Create();
		$this->txtPesoTotal = $this->mctFactura->txtPesoTotal_Create();
		$this->txtDsctoPorPeso = $this->mctFactura->txtDsctoPorPeso_Create();
		$this->txtMontoDsctoPeso = $this->mctFactura->txtMontoDsctoPeso_Create();
		$this->txtTicketFiscal = $this->mctFactura->txtTicketFiscal_Create();
		$this->txtPersonaAutorizada = $this->mctFactura->txtPersonaAutorizada_Create();
		$this->lstImpresaObject = $this->mctFactura->lstImpresaObject_Create();
		$this->lstSistema = $this->mctFactura->lstSistema_Create();
		$this->txtDireccionEntrega = $this->mctFactura->txtDireccionEntrega_Create();
		$this->txtTipoImpresora = $this->mctFactura->txtTipoImpresora_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFactura->Factura && !isset($_SESSION['DataFactura'])) {
            $_SESSION['DataFactura'] = serialize(array($this->mctFactura->Factura));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFactura']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFactura->Factura->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFactura->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Factura'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFactura->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Factura',$this->mctFactura->Factura->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFactura->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFactura->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objFactura = Factura::LoadBySerieNumero($this->txtSerie->Text,$this->txtNumero->Text)) && ($objFactura->Id != $this->mctFactura->Factura->Id )){
				$blnToReturn = false;
				$this->txtSerie->Warning = QApplication::Translate("Already in Use");
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/factura_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacturaMetaControl
		$this->mctFactura->SaveFactura();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFactura->TablasRelacionadasFactura();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacturaMetaControl
			$this->mctFactura->DeleteFactura();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFactura->Factura->Id;
        $_SESSION['TablRefe'] = 'Factura';
        $_SESSION['RegiReto'] = 'factura_edit.php/'.$this->mctFactura->Factura->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/factura_list.php');
    }

 ...
 * NotaCreditoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a NotaCredito columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both nota_credito_edit.php AND
 * nota_credito_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class NotaCreditoEditFormBase extends QForm {
	// Local instance of the NotaCreditoMetaControl
	/**
	 * @var NotaCreditoMetaControlGen mctNotaCredito
	 */
	protected $mctNotaCredito;
	...
	// Controls for NotaCredito's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the NotaCreditoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctNotaCredito = NotaCreditoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on NotaCredito's data fields
		$this->lblId = $this->mctNotaCredito->lblId_Create();
		$this->lstFactura = $this->mctNotaCredito->lstFactura_Create();
		$this->txtCedulaRif = $this->mctNotaCredito->txtCedulaRif_Create();
		$this->txtRazonSocial = $this->mctNotaCredito->txtRazonSocial_Create();
		$this->txtDireccionFiscal = $this->mctNotaCredito->txtDireccionFiscal_Create();
		$this->txtTelefono = $this->mctNotaCredito->txtTelefono_Create();
		...
		$this->txtMontoIva = $this->mctNotaCredito->txtMontoIva_Create();
		$this->txtMontoSeguro = $this->mctNotaCredito->txtMontoSeguro_Create();
		$this->txtMontoOtros = $this->mctNotaCredito->txtMontoOtros_Create();
		$this->txtMontoTotal = $this->mctNotaCredito->txtMontoTotal_Create();
		$this->lstSucursal = $this->mctNotaCredito->lstSucursal_Create();
		$this->lstReceptoria = $this->mctNotaCredito->lstReceptoria_Create();
		$this->lstCaja = $this->mctNotaCredito->lstCaja_Create();
		$this->lstCreadaPorObject = $this->mctNotaCredito->lstCreadaPorObject_Create();
		$this->calCreadaEl = $this->mctNotaCredito->calCreadaEl_Create();
		$this->txtEstatus = $this->mctNotaCredito->txtEstatus_Create();
		$this->lstImpresa = $this->mctNotaCredito->lstImpresa_Create();
		$this->txtMontoDscto = $this->mctNotaCredito->txtMontoDscto_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctNotaCredito->NotaCredito && !isset($_SESSION['DataNotaCredito'])) {
            $_SESSION['DataNotaCredito'] = serialize(array($this->mctNotaCredito->NotaCredito));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataNotaCredito']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctNotaCredito->NotaCredito->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'NotaCredito';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctNotaCredito->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('NotaCredito'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctNotaCredito->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('NotaCredito',$this->mctNotaCredito->NotaCredito->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctNotaCredito->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctNotaCredito->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/nota_credito_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctNotaCredito->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the NotaCreditoMetaControl
		$this->mctNotaCredito->SaveNotaCredito();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctNotaCredito->TablasRelacionadasNotaCredito();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the NotaCreditoMetaControl
			$this->mctNotaCredito->DeleteNotaCredito();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctNotaCredito->NotaCredito->Id;
        $_SESSION['TablRefe'] = 'NotaCredito';
        $_SESSION['RegiReto'] = 'nota_credito_edit.php/'.$this->mctNotaCredito->NotaCredito->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/nota_credito_list.php');
    }

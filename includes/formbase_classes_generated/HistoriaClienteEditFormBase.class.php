 ...
 * HistoriaClienteMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a HistoriaCliente columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both historia_cliente_edit.php AND
 * historia_cliente_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class HistoriaClienteEditFormBase extends QForm {
	// Local instance of the HistoriaClienteMetaControl
	/**
	 * @var HistoriaClienteMetaControlGen mctHistoriaCliente
	 */
	protected $mctHistoriaCliente;
	...
	// Controls for HistoriaCliente's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the HistoriaClienteMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctHistoriaCliente = HistoriaClienteMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on HistoriaCliente's data fields
		$this->lblId = $this->mctHistoriaCliente->lblId_Create();
		$this->lstCliente = $this->mctHistoriaCliente->lstCliente_Create();
		$this->txtComentario = $this->mctHistoriaCliente->txtComentario_Create();
		$this->lstCodiUsuaObject = $this->mctHistoriaCliente->lstCodiUsuaObject_Create();
		$this->lstSucursal = $this->mctHistoriaCliente->lstSucursal_Create();
		$this->calFecha = $this->mctHistoriaCliente->calFecha_Create();
		$this->txtHora = $this->mctHistoriaCliente->txtHora_Create();
		$this->lstCodiCkptObject = $this->mctHistoriaCliente->lstCodiCkptObject_Create();
		$this->chkAlertaCheck = $this->mctHistoriaCliente->chkAlertaCheck_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctHistoriaCliente->HistoriaCliente && !isset($_SESSION['DataHistoriaCliente'])) {
            $_SESSION['DataHistoriaCliente'] = serialize(array($this->mctHistoriaCliente->HistoriaCliente));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataHistoriaCliente']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctHistoriaCliente->HistoriaCliente->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'HistoriaCliente';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctHistoriaCliente->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('HistoriaCliente'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctHistoriaCliente->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('HistoriaCliente',$this->mctHistoriaCliente->HistoriaCliente->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctHistoriaCliente->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctHistoriaCliente->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/historia_cliente_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctHistoriaCliente->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the HistoriaClienteMetaControl
		$this->mctHistoriaCliente->SaveHistoriaCliente();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctHistoriaCliente->TablasRelacionadasHistoriaCliente();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the HistoriaClienteMetaControl
			$this->mctHistoriaCliente->DeleteHistoriaCliente();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctHistoriaCliente->HistoriaCliente->Id;
        $_SESSION['TablRefe'] = 'HistoriaCliente';
        $_SESSION['RegiReto'] = 'historia_cliente_edit.php/'.$this->mctHistoriaCliente->HistoriaCliente->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/historia_cliente_list.php');
    }

 ...
 * of the CargaRecibida class.  It uses the code-generated
 * CargaRecibidaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a CargaRecibida columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both carga_recibida_edit.php AND
 * carga_recibida_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class CargaRecibidaEditFormBase extends QForm {
	// Local instance of the CargaRecibidaMetaControl
	/**
	 * @var CargaRecibidaMetaControlGen mctCargaRecibida
	 */
	protected $mctCargaRecibida;
	...
	// Controls for CargaRecibida's Data Fields
	protected $lblId;
	protected $txtGuiaInternacional;
	protected $calFechaRecepcion;
	protected $txtGuiaLiberty;
	protected $lstCourier;
	protected $lstCliente;
	protected $txtManifiesto;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the CargaRecibidaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctCargaRecibida = CargaRecibidaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on CargaRecibida's data fields
		$this->lblId = $this->mctCargaRecibida->lblId_Create();
		$this->txtGuiaInternacional = $this->mctCargaRecibida->txtGuiaInternacional_Create();
		$this->calFechaRecepcion = $this->mctCargaRecibida->calFechaRecepcion_Create();
		$this->txtGuiaLiberty = $this->mctCargaRecibida->txtGuiaLiberty_Create();
		$this->lstCourier = $this->mctCargaRecibida->lstCourier_Create();
		$this->lstCliente = $this->mctCargaRecibida->lstCliente_Create();
		$this->txtManifiesto = $this->mctCargaRecibida->txtManifiesto_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctCargaRecibida->CargaRecibida && !isset($_SESSION['DataCargaRecibida'])) {
            $_SESSION['DataCargaRecibida'] = serialize(array($this->mctCargaRecibida->CargaRecibida));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataCargaRecibida']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctCargaRecibida->CargaRecibida->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'CargaRecibida';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctCargaRecibida->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('CargaRecibida'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctCargaRecibida->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('CargaRecibida',$this->mctCargaRecibida->CargaRecibida->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctCargaRecibida->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctCargaRecibida->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objCargaRecibida = CargaRecibida::LoadByGuiaInternacional($this->txtGuiaInternacional->Text)) && ($objCargaRecibida->Id != $this->mctCargaRecibida->CargaRecibida->Id )){
				$blnToReturn = false;
				$this->txtGuiaInternacional->Warning = QApplication::Translate("Already in Use");
			}
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/carga_recibida_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctCargaRecibida->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the CargaRecibidaMetaControl
		$this->mctCargaRecibida->SaveCargaRecibida();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctCargaRecibida->TablasRelacionadasCargaRecibida();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the CargaRecibidaMetaControl
			$this->mctCargaRecibida->DeleteCargaRecibida();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctCargaRecibida->CargaRecibida->Id;
        $_SESSION['TablRefe'] = 'CargaRecibida';
        $_SESSION['RegiReto'] = 'carga_recibida_edit.php/'.$this->mctCargaRecibida->CargaRecibida->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/carga_recibida_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/carga_recibida_edit.php');
    }

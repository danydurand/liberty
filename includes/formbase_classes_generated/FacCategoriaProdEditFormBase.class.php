 ...
 * of the FacCategoriaProd class.  It uses the code-generated
 * FacCategoriaProdMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacCategoriaProd columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_categoria_prod_edit.php AND
 * fac_categoria_prod_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacCategoriaProdEditFormBase extends QForm {
	// Local instance of the FacCategoriaProdMetaControl
	/**
	 * @var FacCategoriaProdMetaControlGen mctFacCategoriaProd
	 */
	protected $mctFacCategoriaProd;
	...
	// Controls for FacCategoriaProd's Data Fields
	protected $lblCodiCate;
	protected $txtDescCate;
	protected $txtFranPost;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacCategoriaProdMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacCategoriaProd = FacCategoriaProdMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacCategoriaProd's data fields
		$this->lblCodiCate = $this->mctFacCategoriaProd->lblCodiCate_Create();
		$this->txtDescCate = $this->mctFacCategoriaProd->txtDescCate_Create();
		$this->txtFranPost = $this->mctFacCategoriaProd->txtFranPost_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacCategoriaProd->FacCategoriaProd && !isset($_SESSION['DataFacCategoriaProd'])) {
            $_SESSION['DataFacCategoriaProd'] = serialize(array($this->mctFacCategoriaProd->FacCategoriaProd));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacCategoriaProd']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacCategoriaProd->FacCategoriaProd->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacCategoriaProd';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacCategoriaProd->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacCategoriaProd'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacCategoriaProd->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacCategoriaProd',$this->mctFacCategoriaProd->FacCategoriaProd->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacCategoriaProd->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacCategoriaProd->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_categoria_prod_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacCategoriaProd->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacCategoriaProdMetaControl
		$this->mctFacCategoriaProd->SaveFacCategoriaProd();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacCategoriaProd->TablasRelacionadasFacCategoriaProd();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lblCodiCate->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacCategoriaProdMetaControl
			$this->mctFacCategoriaProd->DeleteFacCategoriaProd();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacCategoriaProd->FacCategoriaProd->Id;
        $_SESSION['TablRefe'] = 'FacCategoriaProd';
        $_SESSION['RegiReto'] = 'fac_categoria_prod_edit.php/'.$this->mctFacCategoriaProd->FacCategoriaProd->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_categoria_prod_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/fac_categoria_prod_edit.php');
    }

 ...
 * ConciliacionBancariaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ConciliacionBancaria columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both conciliacion_bancaria_edit.php AND
 * conciliacion_bancaria_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ConciliacionBancariaEditFormBase extends QForm {
	// Local instance of the ConciliacionBancariaMetaControl
	/**
	 * @var ConciliacionBancariaMetaControlGen mctConciliacionBancaria
	 */
	protected $mctConciliacionBancaria;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ConciliacionBancariaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctConciliacionBancaria = ConciliacionBancariaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ConciliacionBancaria's data fields
		$this->txtCuentaBancariaId = $this->mctConciliacionBancaria->txtCuentaBancariaId_Create();
		$this->txtNumeReci = $this->mctConciliacionBancaria->txtNumeReci_Create();
		$this->calFechaPago = $this->mctConciliacionBancaria->calFechaPago_Create();
		$this->txtMontoDeposito = $this->mctConciliacionBancaria->txtMontoDeposito_Create();
		$this->chkConfirmado = $this->mctConciliacionBancaria->chkConfirmado_Create();
		$this->txtDescripcion = $this->mctConciliacionBancaria->txtDescripcion_Create();
		$this->txtUsuaRegi = $this->mctConciliacionBancaria->txtUsuaRegi_Create();
		$this->calFechRegi = $this->mctConciliacionBancaria->calFechRegi_Create();
		$this->txtHoraRegi = $this->mctConciliacionBancaria->txtHoraRegi_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctConciliacionBancaria->ConciliacionBancaria && !isset($_SESSION['DataConciliacionBancaria'])) {
            $_SESSION['DataConciliacionBancaria'] = serialize(array($this->mctConciliacionBancaria->ConciliacionBancaria));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataConciliacionBancaria']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctConciliacionBancaria->ConciliacionBancaria->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ConciliacionBancaria';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctConciliacionBancaria->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ConciliacionBancaria'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctConciliacionBancaria->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ConciliacionBancaria',$this->mctConciliacionBancaria->ConciliacionBancaria->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctConciliacionBancaria->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctConciliacionBancaria->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/conciliacion_bancaria_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctConciliacionBancaria->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ConciliacionBancariaMetaControl
		$this->mctConciliacionBancaria->SaveConciliacionBancaria();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctConciliacionBancaria->TablasRelacionadasConciliacionBancaria();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtCuentaBancariaId->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->txtNumeReci->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->calFechaPago->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->txtMontoDeposito->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ConciliacionBancariaMetaControl
			$this->mctConciliacionBancaria->DeleteConciliacionBancaria();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctConciliacionBancaria->ConciliacionBancaria->Id;
        $_SESSION['TablRefe'] = 'ConciliacionBancaria';
        $_SESSION['RegiReto'] = 'conciliacion_bancaria_edit.php/'.$this->mctConciliacionBancaria->ConciliacionBancaria->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/conciliacion_bancaria_list.php');
    }

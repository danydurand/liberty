 ...
 * of the FacProducto class.  It uses the code-generated
 * FacProductoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacProducto columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_producto_edit.php AND
 * fac_producto_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacProductoEditFormBase extends QForm {
	// Local instance of the FacProductoMetaControl
	/**
	 * @var FacProductoMetaControlGen mctFacProducto
	 */
	protected $mctFacProducto;
	...
	// Controls for FacProducto's Data Fields
	protected $lblCodiProd;
	protected $txtSiglProd;
	protected $txtDescProd;
	protected $lstTipoProdObject;
	protected $lstCodiCateObject;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacProductoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacProducto = FacProductoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacProducto's data fields
		$this->lblCodiProd = $this->mctFacProducto->lblCodiProd_Create();
		$this->txtSiglProd = $this->mctFacProducto->txtSiglProd_Create();
		$this->txtDescProd = $this->mctFacProducto->txtDescProd_Create();
		$this->lstTipoProdObject = $this->mctFacProducto->lstTipoProdObject_Create();
		$this->lstCodiCateObject = $this->mctFacProducto->lstCodiCateObject_Create();
		$this->lstCodiStatObject = $this->mctFacProducto->lstCodiStatObject_Create();
		$this->txtTextObse = $this->mctFacProducto->txtTextObse_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacProducto->FacProducto && !isset($_SESSION['DataFacProducto'])) {
            $_SESSION['DataFacProducto'] = serialize(array($this->mctFacProducto->FacProducto));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacProducto']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacProducto->FacProducto->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacProducto';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacProducto->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacProducto'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacProducto->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacProducto',$this->mctFacProducto->FacProducto->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacProducto->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacProducto->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objFacProducto = FacProducto::LoadBySiglProd($this->txtSiglProd->Text)) && ($objFacProducto->CodiProd != $this->mctFacProducto->FacProducto->CodiProd )){
				$blnToReturn = false;
				$this->txtSiglProd->Warning = QApplication::Translate("Already in Use");
			}
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_producto_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacProducto->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacProductoMetaControl
		$this->mctFacProducto->SaveFacProducto();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacProducto->TablasRelacionadasFacProducto();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lblCodiProd->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacProductoMetaControl
			$this->mctFacProducto->DeleteFacProducto();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacProducto->FacProducto->Id;
        $_SESSION['TablRefe'] = 'FacProducto';
        $_SESSION['RegiReto'] = 'fac_producto_edit.php/'.$this->mctFacProducto->FacProducto->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_producto_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/fac_producto_edit.php');
    }

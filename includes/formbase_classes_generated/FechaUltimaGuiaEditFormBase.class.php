 ...
 * FechaUltimaGuiaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FechaUltimaGuia columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fecha_ultima_guia_edit.php AND
 * fecha_ultima_guia_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FechaUltimaGuiaEditFormBase extends QForm {
	// Local instance of the FechaUltimaGuiaMetaControl
	/**
	 * @var FechaUltimaGuiaMetaControlGen mctFechaUltimaGuia
	 */
	protected $mctFechaUltimaGuia;
	...
	// Controls for FechaUltimaGuia's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FechaUltimaGuiaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFechaUltimaGuia = FechaUltimaGuiaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FechaUltimaGuia's data fields
		$this->lblId = $this->mctFechaUltimaGuia->lblId_Create();
		$this->lstCliente = $this->mctFechaUltimaGuia->lstCliente_Create();
		$this->calFechaUltimaGuia = $this->mctFechaUltimaGuia->calFechaUltimaGuia_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFechaUltimaGuia->FechaUltimaGuia && !isset($_SESSION['DataFechaUltimaGuia'])) {
            $_SESSION['DataFechaUltimaGuia'] = serialize(array($this->mctFechaUltimaGuia->FechaUltimaGuia));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFechaUltimaGuia']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFechaUltimaGuia->FechaUltimaGuia->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FechaUltimaGuia';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFechaUltimaGuia->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FechaUltimaGuia'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFechaUltimaGuia->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FechaUltimaGuia',$this->mctFechaUltimaGuia->FechaUltimaGuia->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFechaUltimaGuia->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFechaUltimaGuia->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objFechaUltimaGuia = FechaUltimaGuia::LoadByClienteId($this->lstCliente->SelectedValue)) && ($objFechaUltimaGuia->Id != $this->mctFechaUltimaGuia->FechaUltimaGuia->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fecha_ultima_guia_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFechaUltimaGuia->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FechaUltimaGuiaMetaControl
		$this->mctFechaUltimaGuia->SaveFechaUltimaGuia();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFechaUltimaGuia->TablasRelacionadasFechaUltimaGuia();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FechaUltimaGuiaMetaControl
			$this->mctFechaUltimaGuia->DeleteFechaUltimaGuia();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFechaUltimaGuia->FechaUltimaGuia->Id;
        $_SESSION['TablRefe'] = 'FechaUltimaGuia';
        $_SESSION['RegiReto'] = 'fecha_ultima_guia_edit.php/'.$this->mctFechaUltimaGuia->FechaUltimaGuia->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fecha_ultima_guia_list.php');
    }

 ...
 * SaldoExcedenteMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a SaldoExcedente columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both saldo_excedente_edit.php AND
 * saldo_excedente_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class SaldoExcedenteEditFormBase extends QForm {
	// Local instance of the SaldoExcedenteMetaControl
	/**
	 * @var SaldoExcedenteMetaControlGen mctSaldoExcedente
	 */
	protected $mctSaldoExcedente;
	...
	// Controls for SaldoExcedente's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the SaldoExcedenteMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctSaldoExcedente = SaldoExcedenteMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on SaldoExcedente's data fields
		$this->lblId = $this->mctSaldoExcedente->lblId_Create();
		$this->lstCliente = $this->mctSaldoExcedente->lstCliente_Create();
		$this->calFechaRegistro = $this->mctSaldoExcedente->calFechaRegistro_Create();
		$this->txtHora = $this->mctSaldoExcedente->txtHora_Create();
		$this->txtNumeReci = $this->mctSaldoExcedente->txtNumeReci_Create();
		$this->txtMontoDeposito = $this->mctSaldoExcedente->txtMontoDeposito_Create();
		$this->txtSaldoExcedente = $this->mctSaldoExcedente->txtSaldoExcedente_Create();
		$this->lstCodiCkptObject = $this->mctSaldoExcedente->lstCodiCkptObject_Create();
		$this->chkConfirmado = $this->mctSaldoExcedente->chkConfirmado_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctSaldoExcedente->SaldoExcedente && !isset($_SESSION['DataSaldoExcedente'])) {
            $_SESSION['DataSaldoExcedente'] = serialize(array($this->mctSaldoExcedente->SaldoExcedente));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataSaldoExcedente']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctSaldoExcedente->SaldoExcedente->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'SaldoExcedente';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctSaldoExcedente->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('SaldoExcedente'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctSaldoExcedente->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('SaldoExcedente',$this->mctSaldoExcedente->SaldoExcedente->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctSaldoExcedente->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctSaldoExcedente->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/saldo_excedente_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctSaldoExcedente->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the SaldoExcedenteMetaControl
		$this->mctSaldoExcedente->SaveSaldoExcedente();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctSaldoExcedente->TablasRelacionadasSaldoExcedente();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the SaldoExcedenteMetaControl
			$this->mctSaldoExcedente->DeleteSaldoExcedente();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctSaldoExcedente->SaldoExcedente->Id;
        $_SESSION['TablRefe'] = 'SaldoExcedente';
        $_SESSION['RegiReto'] = 'saldo_excedente_edit.php/'.$this->mctSaldoExcedente->SaldoExcedente->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/saldo_excedente_list.php');
    }

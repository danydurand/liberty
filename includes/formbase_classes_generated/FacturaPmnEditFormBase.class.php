 ...
 * FacturaPmnMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacturaPmn columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both factura_pmn_edit.php AND
 * factura_pmn_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacturaPmnEditFormBase extends QForm {
	// Local instance of the FacturaPmnMetaControl
	/**
	 * @var FacturaPmnMetaControlGen mctFacturaPmn
	 */
	protected $mctFacturaPmn;
	...
	// Controls for FacturaPmn's Data Fields
	protected $lblId;
	protected $txtCedulaRif;
	protected $txtRazonSocial;
	protected $txtDireccionFiscal;
	...
	protected $lstCaja;
	protected $lstCreadaPorObject;
	protected $calCreadaEl;
	protected $txtEstatus;
	protected $lstImpresa;
	protected $lstAnuladaPorObject;
	protected $calAnuladaEl;
	protected $txtMotivoAnulacion;
	protected $txtPorcentajeReteIva;
	protected $txtMontoReteIva;
	protected $txtPorcentajeReteIslr;
	protected $txtMontoReteIslr;
	protected $txtMontoDscto;
	protected $txtMontoCobrado;
	protected $txtComprobanteRetencion;
	protected $calFechaComprobante;
	protected $txtComprobanteRetencionIslr;
	protected $calFechaComprobanteIslr;
	protected $txtTieneRetencion;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacturaPmnMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacturaPmn = FacturaPmnMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacturaPmn's data fields
		$this->lblId = $this->mctFacturaPmn->lblId_Create();
		$this->txtCedulaRif = $this->mctFacturaPmn->txtCedulaRif_Create();
		$this->txtRazonSocial = $this->mctFacturaPmn->txtRazonSocial_Create();
		$this->txtDireccionFiscal = $this->mctFacturaPmn->txtDireccionFiscal_Create();
		$this->txtTelefono = $this->mctFacturaPmn->txtTelefono_Create();
		$this->txtNumero = $this->mctFacturaPmn->txtNumero_Create();
		$this->txtMaquinaFiscal = $this->mctFacturaPmn->txtMaquinaFiscal_Create();
		$this->txtFechaImpresion = $this->mctFacturaPmn->txtFechaImpresion_Create();
		$this->txtHoraImpresion = $this->mctFacturaPmn->txtHoraImpresion_Create();
		$this->txtMontoBase = $this->mctFacturaPmn->txtMontoBase_Create();
		$this->txtMontoFranqueo = $this->mctFacturaPmn->txtMontoFranqueo_Create();
		$this->txtMontoIva = $this->mctFacturaPmn->txtMontoIva_Create();
		$this->txtMontoSeguro = $this->mctFacturaPmn->txtMontoSeguro_Create();
		$this->txtMontoOtros = $this->mctFacturaPmn->txtMontoOtros_Create();
		$this->txtMontoTotal = $this->mctFacturaPmn->txtMontoTotal_Create();
		$this->lstSucursal = $this->mctFacturaPmn->lstSucursal_Create();
		$this->txtReceptoriaId = $this->mctFacturaPmn->txtReceptoriaId_Create();
		$this->lstCaja = $this->mctFacturaPmn->lstCaja_Create();
		$this->lstCreadaPorObject = $this->mctFacturaPmn->lstCreadaPorObject_Create();
		$this->calCreadaEl = $this->mctFacturaPmn->calCreadaEl_Create();
		$this->txtEstatus = $this->mctFacturaPmn->txtEstatus_Create();
		$this->lstImpresa = $this->mctFacturaPmn->lstImpresa_Create();
		$this->lstAnuladaPorObject = $this->mctFacturaPmn->lstAnuladaPorObject_Create();
		$this->calAnuladaEl = $this->mctFacturaPmn->calAnuladaEl_Create();
		$this->txtMotivoAnulacion = $this->mctFacturaPmn->txtMotivoAnulacion_Create();
		$this->txtPorcentajeReteIva = $this->mctFacturaPmn->txtPorcentajeReteIva_Create();
		$this->txtMontoReteIva = $this->mctFacturaPmn->txtMontoReteIva_Create();
		$this->txtPorcentajeReteIslr = $this->mctFacturaPmn->txtPorcentajeReteIslr_Create();
		$this->txtMontoReteIslr = $this->mctFacturaPmn->txtMontoReteIslr_Create();
		$this->txtMontoDscto = $this->mctFacturaPmn->txtMontoDscto_Create();
		$this->txtMontoCobrado = $this->mctFacturaPmn->txtMontoCobrado_Create();
		$this->txtComprobanteRetencion = $this->mctFacturaPmn->txtComprobanteRetencion_Create();
		$this->calFechaComprobante = $this->mctFacturaPmn->calFechaComprobante_Create();
		$this->txtComprobanteRetencionIslr = $this->mctFacturaPmn->txtComprobanteRetencionIslr_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacturaPmn->FacturaPmn && !isset($_SESSION['DataFacturaPmn'])) {
            $_SESSION['DataFacturaPmn'] = serialize(array($this->mctFacturaPmn->FacturaPmn));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacturaPmn']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacturaPmn->FacturaPmn->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacturaPmn';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacturaPmn->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacturaPmn'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacturaPmn->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacturaPmn',$this->mctFacturaPmn->FacturaPmn->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacturaPmn->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacturaPmn->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/factura_pmn_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacturaPmn->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacturaPmnMetaControl
		$this->mctFacturaPmn->SaveFacturaPmn();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacturaPmn->TablasRelacionadasFacturaPmn();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacturaPmnMetaControl
			$this->mctFacturaPmn->DeleteFacturaPmn();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacturaPmn->FacturaPmn->Id;
        $_SESSION['TablRefe'] = 'FacturaPmn';
        $_SESSION['RegiReto'] = 'factura_pmn_edit.php/'.$this->mctFacturaPmn->FacturaPmn->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/factura_pmn_list.php');
    }

 ...
 * of the SdeTipoOper class.  It uses the code-generated
 * SdeTipoOperMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a SdeTipoOper columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both sde_tipo_oper_edit.php AND
 * sde_tipo_oper_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class SdeTipoOperEditFormBase extends QForm {
	// Local instance of the SdeTipoOperMetaControl
	/**
	 * @var SdeTipoOperMetaControlGen mctSdeTipoOper
	 */
	protected $mctSdeTipoOper;
	...
	// Controls for SdeTipoOper's Data Fields
	protected $txtCodiTipo;
	protected $txtDescTipo;
	protected $txtStanHora;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the SdeTipoOperMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctSdeTipoOper = SdeTipoOperMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on SdeTipoOper's data fields
		$this->txtCodiTipo = $this->mctSdeTipoOper->txtCodiTipo_Create();
		$this->txtDescTipo = $this->mctSdeTipoOper->txtDescTipo_Create();
		$this->txtStanHora = $this->mctSdeTipoOper->txtStanHora_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctSdeTipoOper->SdeTipoOper && !isset($_SESSION['DataSdeTipoOper'])) {
            $_SESSION['DataSdeTipoOper'] = serialize(array($this->mctSdeTipoOper->SdeTipoOper));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataSdeTipoOper']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctSdeTipoOper->SdeTipoOper->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'SdeTipoOper';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctSdeTipoOper->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('SdeTipoOper'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctSdeTipoOper->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('SdeTipoOper',$this->mctSdeTipoOper->SdeTipoOper->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctSdeTipoOper->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctSdeTipoOper->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objSdeTipoOper = SdeTipoOper::LoadByDescTipo($this->txtDescTipo->Text)) && ($objSdeTipoOper->CodiTipo != $this->mctSdeTipoOper->SdeTipoOper->CodiTipo )){
				$blnToReturn = false;
				$this->txtDescTipo->Warning = QApplication::Translate("Already in Use");
			}
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/sde_tipo_oper_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctSdeTipoOper->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the SdeTipoOperMetaControl
		$this->mctSdeTipoOper->SaveSdeTipoOper();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctSdeTipoOper->TablasRelacionadasSdeTipoOper();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtCodiTipo->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the SdeTipoOperMetaControl
			$this->mctSdeTipoOper->DeleteSdeTipoOper();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctSdeTipoOper->SdeTipoOper->Id;
        $_SESSION['TablRefe'] = 'SdeTipoOper';
        $_SESSION['RegiReto'] = 'sde_tipo_oper_edit.php/'.$this->mctSdeTipoOper->SdeTipoOper->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/sde_tipo_oper_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/sde_tipo_oper_edit.php');
    }

 ...
 * of the RegistroTrabajo class.  It uses the code-generated
 * RegistroTrabajoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a RegistroTrabajo columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both registro_trabajo_edit.php AND
 * registro_trabajo_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class RegistroTrabajoEditFormBase extends QForm {
	// Local instance of the RegistroTrabajoMetaControl
	/**
	 * @var RegistroTrabajoMetaControlGen mctRegistroTrabajo
	 */
	protected $mctRegistroTrabajo;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the RegistroTrabajoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctRegistroTrabajo = RegistroTrabajoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on RegistroTrabajo's data fields
		$this->lblId = $this->mctRegistroTrabajo->lblId_Create();
		$this->lstGuia = $this->mctRegistroTrabajo->lstGuia_Create();
		$this->txtComentario = $this->mctRegistroTrabajo->txtComentario_Create();
		$this->lstUsuario = $this->mctRegistroTrabajo->lstUsuario_Create();
		$this->calFecha = $this->mctRegistroTrabajo->calFecha_Create();
		$this->txtHora = $this->mctRegistroTrabajo->txtHora_Create();
		$this->lstSucursal = $this->mctRegistroTrabajo->lstSucursal_Create();
		$this->txtCheckpointId = $this->mctRegistroTrabajo->txtCheckpointId_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctRegistroTrabajo->RegistroTrabajo && !isset($_SESSION['DataRegistroTrabajo'])) {
            $_SESSION['DataRegistroTrabajo'] = serialize(array($this->mctRegistroTrabajo->RegistroTrabajo));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataRegistroTrabajo']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctRegistroTrabajo->RegistroTrabajo->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'RegistroTrabajo';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctRegistroTrabajo->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('RegistroTrabajo'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctRegistroTrabajo->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('RegistroTrabajo',$this->mctRegistroTrabajo->RegistroTrabajo->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctRegistroTrabajo->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctRegistroTrabajo->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/registro_trabajo_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctRegistroTrabajo->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the RegistroTrabajoMetaControl
		$this->mctRegistroTrabajo->SaveRegistroTrabajo();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctRegistroTrabajo->TablasRelacionadasRegistroTrabajo();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the RegistroTrabajoMetaControl
			$this->mctRegistroTrabajo->DeleteRegistroTrabajo();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctRegistroTrabajo->RegistroTrabajo->Id;
        $_SESSION['TablRefe'] = 'RegistroTrabajo';
        $_SESSION['RegiReto'] = 'registro_trabajo_edit.php/'.$this->mctRegistroTrabajo->RegistroTrabajo->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/registro_trabajo_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/registro_trabajo_edit.php');
    }

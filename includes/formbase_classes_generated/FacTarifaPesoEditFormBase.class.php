 ...
 * FacTarifaPesoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacTarifaPeso columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_tarifa_peso_edit.php AND
 * fac_tarifa_peso_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacTarifaPesoEditFormBase extends QForm {
	// Local instance of the FacTarifaPesoMetaControl
	/**
	 * @var FacTarifaPesoMetaControlGen mctFacTarifaPeso
	 */
	protected $mctFacTarifaPeso;
	...
	// Controls for FacTarifaPeso's Data Fields
	protected $lblId;
	protected $lstTarifa;
	protected $lstCodiProdObject;
	protected $lstOrigenObject;
	protected $lstDestinoObject;
	protected $txtPesoInicial;
	protected $txtPesoFinal;
	protected $txtMontoTarifa;
	protected $txtFranqueoPostal;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacTarifaPesoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacTarifaPeso = FacTarifaPesoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacTarifaPeso's data fields
		$this->lblId = $this->mctFacTarifaPeso->lblId_Create();
		$this->lstTarifa = $this->mctFacTarifaPeso->lstTarifa_Create();
		$this->lstCodiProdObject = $this->mctFacTarifaPeso->lstCodiProdObject_Create();
		$this->lstOrigenObject = $this->mctFacTarifaPeso->lstOrigenObject_Create();
		$this->lstDestinoObject = $this->mctFacTarifaPeso->lstDestinoObject_Create();
		$this->txtPesoInicial = $this->mctFacTarifaPeso->txtPesoInicial_Create();
		$this->txtPesoFinal = $this->mctFacTarifaPeso->txtPesoFinal_Create();
		$this->txtMontoTarifa = $this->mctFacTarifaPeso->txtMontoTarifa_Create();
		$this->txtFranqueoPostal = $this->mctFacTarifaPeso->txtFranqueoPostal_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacTarifaPeso->FacTarifaPeso && !isset($_SESSION['DataFacTarifaPeso'])) {
            $_SESSION['DataFacTarifaPeso'] = serialize(array($this->mctFacTarifaPeso->FacTarifaPeso));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacTarifaPeso']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacTarifaPeso->FacTarifaPeso->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacTarifaPeso';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacTarifaPeso->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacTarifaPeso'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacTarifaPeso->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacTarifaPeso',$this->mctFacTarifaPeso->FacTarifaPeso->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacTarifaPeso->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacTarifaPeso->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objFacTarifaPeso = FacTarifaPeso::LoadByTarifaIdCodiProdOrigenDestinoPesoInicialPesoFinal($this->lstTarifa->SelectedValue,$this->lstCodiProdObject->SelectedValue,$this->lstOrigenObject->SelectedValue,$this->lstDestinoObject->SelectedValue,$this->txtPesoInicial->Text,$this->txtPesoFinal->Text)) && ($objFacTarifaPeso->Id != $this->mctFacTarifaPeso->FacTarifaPeso->Id )){
				$blnToReturn = false;
				$this->lstTarifa->Warning = QApplication::Translate("Already in Use");
				$this->lstCodiProdObject->Warning = QApplication::Translate("Already in Use");
				$this->lstOrigenObject->Warning = QApplication::Translate("Already in Use");
				$this->lstDestinoObject->Warning = QApplication::Translate("Already in Use");
				$this->txtPesoInicial->Warning = QApplication::Translate("Already in Use");
				$this->txtPesoFinal->Warning = QApplication::Translate("Already in Use");
			}
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_tarifa_peso_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacTarifaPeso->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacTarifaPesoMetaControl
		$this->mctFacTarifaPeso->SaveFacTarifaPeso();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacTarifaPeso->TablasRelacionadasFacTarifaPeso();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacTarifaPesoMetaControl
			$this->mctFacTarifaPeso->DeleteFacTarifaPeso();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacTarifaPeso->FacTarifaPeso->Id;
        $_SESSION['TablRefe'] = 'FacTarifaPeso';
        $_SESSION['RegiReto'] = 'fac_tarifa_peso_edit.php/'.$this->mctFacTarifaPeso->FacTarifaPeso->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_tarifa_peso_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/fac_tarifa_peso_edit.php');
    }

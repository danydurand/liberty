 ...
 * of the PagoMail class.  It uses the code-generated
 * PagoMailMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a PagoMail columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both pago_mail_edit.php AND
 * pago_mail_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class PagoMailEditFormBase extends QForm {
	// Local instance of the PagoMailMetaControl
	/**
	 * @var PagoMailMetaControlGen mctPagoMail
	 */
	protected $mctPagoMail;
	...
	// Controls for PagoMail's Data Fields
	protected $lblId;
	protected $txtClienteId;
	protected $txtCedula;
	protected $txtFechaTransaccion;
	protected $txtFormaPago;
	protected $txtNumeroVoucher;
	protected $txtMontoDepositado;
	protected $txtGuiaId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the PagoMailMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctPagoMail = PagoMailMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on PagoMail's data fields
		$this->lblId = $this->mctPagoMail->lblId_Create();
		$this->txtClienteId = $this->mctPagoMail->txtClienteId_Create();
		$this->txtCedula = $this->mctPagoMail->txtCedula_Create();
		$this->txtFechaTransaccion = $this->mctPagoMail->txtFechaTransaccion_Create();
		$this->txtFormaPago = $this->mctPagoMail->txtFormaPago_Create();
		$this->txtNumeroVoucher = $this->mctPagoMail->txtNumeroVoucher_Create();
		$this->txtMontoDepositado = $this->mctPagoMail->txtMontoDepositado_Create();
		$this->txtGuiaId = $this->mctPagoMail->txtGuiaId_Create();
		$this->txtFechaEmail = $this->mctPagoMail->txtFechaEmail_Create();
		$this->calFechaRegistro = $this->mctPagoMail->calFechaRegistro_Create();
		$this->txtHoraRegistro = $this->mctPagoMail->txtHoraRegistro_Create();
		$this->txtEmail = $this->mctPagoMail->txtEmail_Create();
		$this->txtProcesado = $this->mctPagoMail->txtProcesado_Create();
		$this->calFechaProcesado = $this->mctPagoMail->calFechaProcesado_Create();
		$this->txtHoraProcesado = $this->mctPagoMail->txtHoraProcesado_Create();
		$this->txtStatus = $this->mctPagoMail->txtStatus_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctPagoMail->PagoMail && !isset($_SESSION['DataPagoMail'])) {
            $_SESSION['DataPagoMail'] = serialize(array($this->mctPagoMail->PagoMail));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataPagoMail']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctPagoMail->PagoMail->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'PagoMail';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctPagoMail->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('PagoMail'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctPagoMail->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('PagoMail',$this->mctPagoMail->PagoMail->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctPagoMail->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctPagoMail->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/pago_mail_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the PagoMailMetaControl
		$this->mctPagoMail->SavePagoMail();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctPagoMail->TablasRelacionadasPagoMail();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the PagoMailMetaControl
			$this->mctPagoMail->DeletePagoMail();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctPagoMail->PagoMail->Id;
        $_SESSION['TablRefe'] = 'PagoMail';
        $_SESSION['RegiReto'] = 'pago_mail_edit.php/'.$this->mctPagoMail->PagoMail->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/pago_mail_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/pago_mail_edit.php');
    }

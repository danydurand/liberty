 ...
 * DatosPagoMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a DatosPago columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both datos_pago_edit.php AND
 * datos_pago_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class DatosPagoEditFormBase extends QForm {
	// Local instance of the DatosPagoMetaControl
	/**
	 * @var DatosPagoMetaControlGen mctDatosPago
	 */
	protected $mctDatosPago;
	...
	// Controls for DatosPago's Data Fields
	protected $lblId;
	protected $lstCliente;
	protected $txtNumeReci;
	protected $txtGuiaId;
	protected $calFechaRegistro;
	protected $txtHora;
	protected $calFechaPago;
	protected $txtCuentaCliente;
	protected $txtTipoTransaccionId;
	protected $lstCuentaBancaria;
	protected $txtMontoDeposito;
	protected $txtMontoUsado;
	protected $chkConfirmado;
	protected $lstCodiCkptObject;
	protected $lstUsuario;
	protected $lstSaldoExcedente;
	protected $calFechConf;
	protected $txtUsuaConf;
	protected $txtHoraConf;
	protected $txtDifeBanc;
	protected $txtMatched;
	protected $txtTipoConc;
	protected $txtBancOrig;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the DatosPagoMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctDatosPago = DatosPagoMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on DatosPago's data fields
		$this->lblId = $this->mctDatosPago->lblId_Create();
		$this->lstCliente = $this->mctDatosPago->lstCliente_Create();
		$this->txtNumeReci = $this->mctDatosPago->txtNumeReci_Create();
		$this->txtGuiaId = $this->mctDatosPago->txtGuiaId_Create();
		$this->calFechaRegistro = $this->mctDatosPago->calFechaRegistro_Create();
		$this->txtHora = $this->mctDatosPago->txtHora_Create();
		$this->calFechaPago = $this->mctDatosPago->calFechaPago_Create();
		$this->txtCuentaCliente = $this->mctDatosPago->txtCuentaCliente_Create();
		$this->txtTipoTransaccionId = $this->mctDatosPago->txtTipoTransaccionId_Create();
		$this->lstCuentaBancaria = $this->mctDatosPago->lstCuentaBancaria_Create();
		$this->txtMontoDeposito = $this->mctDatosPago->txtMontoDeposito_Create();
		$this->txtMontoUsado = $this->mctDatosPago->txtMontoUsado_Create();
		$this->chkConfirmado = $this->mctDatosPago->chkConfirmado_Create();
		$this->lstCodiCkptObject = $this->mctDatosPago->lstCodiCkptObject_Create();
		$this->lstUsuario = $this->mctDatosPago->lstUsuario_Create();
		$this->lstSaldoExcedente = $this->mctDatosPago->lstSaldoExcedente_Create();
		$this->calFechConf = $this->mctDatosPago->calFechConf_Create();
		$this->txtUsuaConf = $this->mctDatosPago->txtUsuaConf_Create();
		$this->txtHoraConf = $this->mctDatosPago->txtHoraConf_Create();
		$this->txtDifeBanc = $this->mctDatosPago->txtDifeBanc_Create();
		$this->txtMatched = $this->mctDatosPago->txtMatched_Create();
		$this->txtTipoConc = $this->mctDatosPago->txtTipoConc_Create();
		$this->txtBancOrig = $this->mctDatosPago->txtBancOrig_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctDatosPago->DatosPago && !isset($_SESSION['DataDatosPago'])) {
            $_SESSION['DataDatosPago'] = serialize(array($this->mctDatosPago->DatosPago));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataDatosPago']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctDatosPago->DatosPago->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'DatosPago';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctDatosPago->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('DatosPago'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctDatosPago->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('DatosPago',$this->mctDatosPago->DatosPago->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctDatosPago->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctDatosPago->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objDatosPago = DatosPago::LoadByClienteIdNumeReciGuiaId($this->lstCliente->SelectedValue,$this->txtNumeReci->Text,$this->txtGuiaId->Text)) && ($objDatosPago->Id != $this->mctDatosPago->DatosPago->Id )){
				$blnToReturn = false;
				$this->lstCliente->Warning = QApplication::Translate("Already in Use");
				$this->txtNumeReci->Warning = QApplication::Translate("Already in Use");
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/datos_pago_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the DatosPagoMetaControl
		$this->mctDatosPago->SaveDatosPago();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctDatosPago->TablasRelacionadasDatosPago();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the DatosPagoMetaControl
			$this->mctDatosPago->DeleteDatosPago();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctDatosPago->DatosPago->Id;
        $_SESSION['TablRefe'] = 'DatosPago';
        $_SESSION['RegiReto'] = 'datos_pago_edit.php/'.$this->mctDatosPago->DatosPago->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/datos_pago_list.php');
    }

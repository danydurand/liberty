 ...
 * of the FacTarifaProd class.  It uses the code-generated
 * FacTarifaProdMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacTarifaProd columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_tarifa_prod_edit.php AND
 * fac_tarifa_prod_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacTarifaProdEditFormBase extends QForm {
	// Local instance of the FacTarifaProdMetaControl
	/**
	 * @var FacTarifaProdMetaControlGen mctFacTarifaProd
	 */
	protected $mctFacTarifaProd;
	...
	// Controls for FacTarifaProd's Data Fields
	protected $lblCodiTari;
	protected $lstCodiClieObject;
	protected $lstCodiProdObject;
	protected $txtMontBase;
	protected $txtFormConc;
	protected $lstTipoOperObject;
	protected $txtOrdeImpr;
	protected $txtEtiqFact;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacTarifaProdMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacTarifaProd = FacTarifaProdMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacTarifaProd's data fields
		$this->lblCodiTari = $this->mctFacTarifaProd->lblCodiTari_Create();
		$this->lstCodiClieObject = $this->mctFacTarifaProd->lstCodiClieObject_Create();
		$this->lstCodiProdObject = $this->mctFacTarifaProd->lstCodiProdObject_Create();
		$this->txtMontBase = $this->mctFacTarifaProd->txtMontBase_Create();
		$this->txtFormConc = $this->mctFacTarifaProd->txtFormConc_Create();
		$this->lstTipoOperObject = $this->mctFacTarifaProd->lstTipoOperObject_Create();
		$this->txtOrdeImpr = $this->mctFacTarifaProd->txtOrdeImpr_Create();
		$this->txtEtiqFact = $this->mctFacTarifaProd->txtEtiqFact_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacTarifaProd->FacTarifaProd && !isset($_SESSION['DataFacTarifaProd'])) {
            $_SESSION['DataFacTarifaProd'] = serialize(array($this->mctFacTarifaProd->FacTarifaProd));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacTarifaProd']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacTarifaProd->FacTarifaProd->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacTarifaProd';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacTarifaProd->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacTarifaProd'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacTarifaProd->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacTarifaProd',$this->mctFacTarifaProd->FacTarifaProd->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacTarifaProd->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacTarifaProd->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_tarifa_prod_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacTarifaProd->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacTarifaProdMetaControl
		$this->mctFacTarifaProd->SaveFacTarifaProd();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacTarifaProd->TablasRelacionadasFacTarifaProd();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lblCodiTari->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->lstCodiClieObject->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->lstCodiProdObject->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacTarifaProdMetaControl
			$this->mctFacTarifaProd->DeleteFacTarifaProd();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacTarifaProd->FacTarifaProd->Id;
        $_SESSION['TablRefe'] = 'FacTarifaProd';
        $_SESSION['RegiReto'] = 'fac_tarifa_prod_edit.php/'.$this->mctFacTarifaProd->FacTarifaProd->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_tarifa_prod_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/fac_tarifa_prod_edit.php');
    }

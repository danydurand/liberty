 ...
 * of the FacTariMasi class.  It uses the code-generated
 * FacTariMasiMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a FacTariMasi columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both fac_tari_masi_edit.php AND
 * fac_tari_masi_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class FacTariMasiEditFormBase extends QForm {
	// Local instance of the FacTariMasiMetaControl
	/**
	 * @var FacTariMasiMetaControlGen mctFacTariMasi
	 */
	protected $mctFacTariMasi;
	...
	// Controls for FacTariMasi's Data Fields
	protected $lstCodiProdObject;
	protected $lstCodiClieObject;
	protected $lstTipoRutaObject;
	protected $txtCantDias;
	protected $txtCobrEntr;
	protected $txtCobrInci;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the FacTariMasiMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctFacTariMasi = FacTariMasiMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on FacTariMasi's data fields
		$this->lstCodiProdObject = $this->mctFacTariMasi->lstCodiProdObject_Create();
		$this->lstCodiClieObject = $this->mctFacTariMasi->lstCodiClieObject_Create();
		$this->lstTipoRutaObject = $this->mctFacTariMasi->lstTipoRutaObject_Create();
		$this->txtCantDias = $this->mctFacTariMasi->txtCantDias_Create();
		$this->txtCobrEntr = $this->mctFacTariMasi->txtCobrEntr_Create();
		$this->txtCobrInci = $this->mctFacTariMasi->txtCobrInci_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctFacTariMasi->FacTariMasi && !isset($_SESSION['DataFacTariMasi'])) {
            $_SESSION['DataFacTariMasi'] = serialize(array($this->mctFacTariMasi->FacTariMasi));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataFacTariMasi']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctFacTariMasi->FacTariMasi->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'FacTariMasi';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctFacTariMasi->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('FacTariMasi'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctFacTariMasi->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('FacTariMasi',$this->mctFacTariMasi->FacTariMasi->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctFacTariMasi->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctFacTariMasi->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/fac_tari_masi_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctFacTariMasi->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the FacTariMasiMetaControl
		$this->mctFacTariMasi->SaveFacTariMasi();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctFacTariMasi->TablasRelacionadasFacTariMasi();
		if (count($arrTablRela)) {
			...
			//$this->lstTipoRutaObject->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the FacTariMasiMetaControl
			$this->mctFacTariMasi->DeleteFacTariMasi();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctFacTariMasi->FacTariMasi->Id;
        $_SESSION['TablRefe'] = 'FacTariMasi';
        $_SESSION['RegiReto'] = 'fac_tari_masi_edit.php/'.$this->mctFacTariMasi->FacTariMasi->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/fac_tari_masi_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/fac_tari_masi_edit.php');
    }

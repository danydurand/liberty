 ...
 * TarifaIMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a TarifaI columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both tarifa_i_edit.php AND
 * tarifa_i_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class TarifaIEditFormBase extends QForm {
	// Local instance of the TarifaIMetaControl
	/**
	 * @var TarifaIMetaControlGen mctTarifaI
	 */
	protected $mctTarifaI;
	...
	// Controls for TarifaI's Data Fields
	protected $lblId;
	protected $lstProducto;
	protected $txtMinimoVolumen;
	protected $txtTarifaMinimaVolumen;
	protected $txtLimiteVolumen;
	protected $txtTarifaLibraVolumenAdicional;
	protected $txtSeguroVolumen;
	protected $lstAplicaSgroVolu;
	protected $txtMinimaLibra;
	protected $txtTarifaMinimaLibra;
	protected $txtTarifaLibraPesoAdicional;
	protected $txtSeguroLibraPeso;
	protected $lstAplicaSgroPeso;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the TarifaIMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctTarifaI = TarifaIMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on TarifaI's data fields
		$this->lblId = $this->mctTarifaI->lblId_Create();
		$this->lstProducto = $this->mctTarifaI->lstProducto_Create();
		$this->txtMinimoVolumen = $this->mctTarifaI->txtMinimoVolumen_Create();
		$this->txtTarifaMinimaVolumen = $this->mctTarifaI->txtTarifaMinimaVolumen_Create();
		$this->txtLimiteVolumen = $this->mctTarifaI->txtLimiteVolumen_Create();
		$this->txtTarifaLibraVolumenAdicional = $this->mctTarifaI->txtTarifaLibraVolumenAdicional_Create();
		$this->txtSeguroVolumen = $this->mctTarifaI->txtSeguroVolumen_Create();
		$this->lstAplicaSgroVolu = $this->mctTarifaI->lstAplicaSgroVolu_Create();
		$this->txtMinimaLibra = $this->mctTarifaI->txtMinimaLibra_Create();
		$this->txtTarifaMinimaLibra = $this->mctTarifaI->txtTarifaMinimaLibra_Create();
		$this->txtTarifaLibraPesoAdicional = $this->mctTarifaI->txtTarifaLibraPesoAdicional_Create();
		$this->txtSeguroLibraPeso = $this->mctTarifaI->txtSeguroLibraPeso_Create();
		$this->lstAplicaSgroPeso = $this->mctTarifaI->lstAplicaSgroPeso_Create();
		$this->calFechaVigencia = $this->mctTarifaI->calFechaVigencia_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctTarifaI->TarifaI && !isset($_SESSION['DataTarifaI'])) {
            $_SESSION['DataTarifaI'] = serialize(array($this->mctTarifaI->TarifaI));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataTarifaI']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctTarifaI->TarifaI->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctTarifaI->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('TarifaI'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctTarifaI->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('TarifaI',$this->mctTarifaI->TarifaI->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctTarifaI->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctTarifaI->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objTarifaI = TarifaI::LoadByProductoIdFechaVigencia($this->lstProducto->SelectedValue,$this->calFechaVigencia->DateTime)) && ($objTarifaI->Id != $this->mctTarifaI->TarifaI->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/tarifa_i_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the TarifaIMetaControl
		$this->mctTarifaI->SaveTarifaI();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctTarifaI->TablasRelacionadasTarifaI();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the TarifaIMetaControl
			$this->mctTarifaI->DeleteTarifaI();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctTarifaI->TarifaI->Id;
        $_SESSION['TablRefe'] = 'TarifaI';
        $_SESSION['RegiReto'] = 'tarifa_i_edit.php/'.$this->mctTarifaI->TarifaI->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/tarifa_i_list.php');
    }

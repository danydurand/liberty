 ...
 * of the Compra class.  It uses the code-generated
 * CompraMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a Compra columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both compra_edit.php AND
 * compra_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class CompraEditFormBase extends QForm {
	// Local instance of the CompraMetaControl
	/**
	 * @var CompraMetaControlGen mctCompra
	 */
	protected $mctCompra;
	...
	// Controls for Compra's Data Fields
	protected $lblId;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the CompraMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctCompra = CompraMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on Compra's data fields
		$this->lblId = $this->mctCompra->lblId_Create();
		$this->lstCliente = $this->mctCompra->lstCliente_Create();
		$this->lstCourier = $this->mctCompra->lstCourier_Create();
		$this->txtGuiaInternacional = $this->mctCompra->txtGuiaInternacional_Create();
		$this->calFechaRegistro = $this->mctCompra->calFechaRegistro_Create();
		$this->txtDescripcion = $this->mctCompra->txtDescripcion_Create();
		$this->lstProcesadaObject = $this->mctCompra->lstProcesadaObject_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctCompra->Compra && !isset($_SESSION['DataCompra'])) {
            $_SESSION['DataCompra'] = serialize(array($this->mctCompra->Compra));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataCompra']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctCompra->Compra->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctCompra->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Compra'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctCompra->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Compra',$this->mctCompra->Compra->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctCompra->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctCompra->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objCompra = Compra::LoadByCourierIdGuiaInternacional($this->lstCourier->SelectedValue,$this->txtGuiaInternacional->Text)) && ($objCompra->Id != $this->mctCompra->Compra->Id )){
				$blnToReturn = false;
				$this->lstCourier->Warning = QApplication::Translate("Already in Use");
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/compra_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the CompraMetaControl
		$this->mctCompra->SaveCompra();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctCompra->TablasRelacionadasCompra();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the CompraMetaControl
			$this->mctCompra->DeleteCompra();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctCompra->Compra->Id;
        $_SESSION['TablRefe'] = 'Compra';
        $_SESSION['RegiReto'] = 'compra_edit.php/'.$this->mctCompra->Compra->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/compra_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/compra_edit.php');
    }

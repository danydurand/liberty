 ...
 * of the ZncPorSucursal class.  It uses the code-generated
 * ZncPorSucursalMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ZncPorSucursal columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both znc_por_sucursal_edit.php AND
 * znc_por_sucursal_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ZncPorSucursalEditFormBase extends QForm {
	// Local instance of the ZncPorSucursalMetaControl
	/**
	 * @var ZncPorSucursalMetaControlGen mctZncPorSucursal
	 */
	protected $mctZncPorSucursal;
	...
	// Controls for ZncPorSucursal's Data Fields
	protected $txtSucursalId;
	protected $txtZonasNoCubiertas;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ZncPorSucursalMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctZncPorSucursal = ZncPorSucursalMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ZncPorSucursal's data fields
		$this->txtSucursalId = $this->mctZncPorSucursal->txtSucursalId_Create();
		$this->txtZonasNoCubiertas = $this->mctZncPorSucursal->txtZonasNoCubiertas_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctZncPorSucursal->ZncPorSucursal && !isset($_SESSION['DataZncPorSucursal'])) {
            $_SESSION['DataZncPorSucursal'] = serialize(array($this->mctZncPorSucursal->ZncPorSucursal));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataZncPorSucursal']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctZncPorSucursal->ZncPorSucursal->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ZncPorSucursal';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctZncPorSucursal->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ZncPorSucursal'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctZncPorSucursal->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ZncPorSucursal',$this->mctZncPorSucursal->ZncPorSucursal->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctZncPorSucursal->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctZncPorSucursal->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/znc_por_sucursal_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctZncPorSucursal->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ZncPorSucursalMetaControl
		$this->mctZncPorSucursal->SaveZncPorSucursal();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctZncPorSucursal->TablasRelacionadasZncPorSucursal();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->txtSucursalId->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ZncPorSucursalMetaControl
			$this->mctZncPorSucursal->DeleteZncPorSucursal();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctZncPorSucursal->ZncPorSucursal->Id;
        $_SESSION['TablRefe'] = 'ZncPorSucursal';
        $_SESSION['RegiReto'] = 'znc_por_sucursal_edit.php/'.$this->mctZncPorSucursal->ZncPorSucursal->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/znc_por_sucursal_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/znc_por_sucursal_edit.php');
    }

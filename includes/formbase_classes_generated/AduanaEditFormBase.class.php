 ...
 * of the Aduana class.  It uses the code-generated
 * AduanaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a Aduana columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both aduana_edit.php AND
 * aduana_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class AduanaEditFormBase extends QForm {
	// Local instance of the AduanaMetaControl
	/**
	 * @var AduanaMetaControlGen mctAduana
	 */
	protected $mctAduana;
	...
	// Controls for Aduana's Data Fields
	protected $lstGuia;
	protected $txtMontoFlete;
	protected $txtValorCif;
	protected $txtPorcentajeArancel;
	protected $txtMontoArancel;
	protected $txtPorcentajeTasa;
	protected $txtMontoTasa;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the AduanaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctAduana = AduanaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on Aduana's data fields
		$this->lstGuia = $this->mctAduana->lstGuia_Create();
		$this->txtMontoFlete = $this->mctAduana->txtMontoFlete_Create();
		$this->txtValorCif = $this->mctAduana->txtValorCif_Create();
		$this->txtPorcentajeArancel = $this->mctAduana->txtPorcentajeArancel_Create();
		$this->txtMontoArancel = $this->mctAduana->txtMontoArancel_Create();
		$this->txtPorcentajeTasa = $this->mctAduana->txtPorcentajeTasa_Create();
		$this->txtMontoTasa = $this->mctAduana->txtMontoTasa_Create();
		$this->txtPorcentajeSeguro = $this->mctAduana->txtPorcentajeSeguro_Create();
		$this->txtMontoSeguro = $this->mctAduana->txtMontoSeguro_Create();
		$this->txtPorcentajeIva = $this->mctAduana->txtPorcentajeIva_Create();
		$this->txtMontoIva = $this->mctAduana->txtMontoIva_Create();
		$this->txtTotalAduana = $this->mctAduana->txtTotalAduana_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctAduana->Aduana && !isset($_SESSION['DataAduana'])) {
            $_SESSION['DataAduana'] = serialize(array($this->mctAduana->Aduana));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataAduana']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctAduana->Aduana->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctAduana->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Aduana'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctAduana->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Aduana',$this->mctAduana->Aduana->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctAduana->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctAduana->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/aduana_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the AduanaMetaControl
		$this->mctAduana->SaveAduana();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctAduana->TablasRelacionadasAduana();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the AduanaMetaControl
			$this->mctAduana->DeleteAduana();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctAduana->Aduana->Id;
        $_SESSION['TablRefe'] = 'Aduana';
        $_SESSION['RegiReto'] = 'aduana_edit.php/'.$this->mctAduana->Aduana->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/aduana_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/aduana_edit.php');
    }

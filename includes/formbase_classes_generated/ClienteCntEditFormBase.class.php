 ...
 * of the ClienteCnt class.  It uses the code-generated
 * ClienteCntMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a ClienteCnt columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both cliente_cnt_edit.php AND
 * cliente_cnt_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ClienteCntEditFormBase extends QForm {
	// Local instance of the ClienteCntMetaControl
	/**
	 * @var ClienteCntMetaControlGen mctClienteCnt
	 */
	protected $mctClienteCnt;
	...
	// Controls for ClienteCnt's Data Fields
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ClienteCntMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctClienteCnt = ClienteCntMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on ClienteCnt's data fields
		$this->lstTipoDocumento = $this->mctClienteCnt->lstTipoDocumento_Create();
		$this->txtCedulaRif = $this->mctClienteCnt->txtCedulaRif_Create();
		$this->txtNombre = $this->mctClienteCnt->txtNombre_Create();
		$this->txtDireccion = $this->mctClienteCnt->txtDireccion_Create();
		$this->txtTelefono = $this->mctClienteCnt->txtTelefono_Create();
		$this->txtEmail = $this->mctClienteCnt->txtEmail_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctClienteCnt->ClienteCnt && !isset($_SESSION['DataClienteCnt'])) {
            $_SESSION['DataClienteCnt'] = serialize(array($this->mctClienteCnt->ClienteCnt));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataClienteCnt']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctClienteCnt->ClienteCnt->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'ClienteCnt';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctClienteCnt->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('ClienteCnt'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctClienteCnt->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('ClienteCnt',$this->mctClienteCnt->ClienteCnt->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctClienteCnt->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctClienteCnt->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/cliente_cnt_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctClienteCnt->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ClienteCntMetaControl
		$this->mctClienteCnt->SaveClienteCnt();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctClienteCnt->TablasRelacionadasClienteCnt();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lstTipoDocumento->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
				
			//$this->txtCedulaRif->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ClienteCntMetaControl
			$this->mctClienteCnt->DeleteClienteCnt();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctClienteCnt->ClienteCnt->Id;
        $_SESSION['TablRefe'] = 'ClienteCnt';
        $_SESSION['RegiReto'] = 'cliente_cnt_edit.php/'.$this->mctClienteCnt->ClienteCnt->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/cliente_cnt_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/cliente_cnt_edit.php');
    }

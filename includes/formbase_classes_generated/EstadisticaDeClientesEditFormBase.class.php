 ...
 * EstadisticaDeClientesMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a EstadisticaDeClientes columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both estadistica_de_clientes_edit.php AND
 * estadistica_de_clientes_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class EstadisticaDeClientesEditFormBase extends QForm {
	// Local instance of the EstadisticaDeClientesMetaControl
	/**
	 * @var EstadisticaDeClientesMetaControlGen mctEstadisticaDeClientes
	 */
	protected $mctEstadisticaDeClientes;
	...
	// Controls for EstadisticaDeClientes's Data Fields
	protected $lstCliente;
	protected $calFechaPrimeraGuia;
	protected $calFechaUltimaGuia;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the EstadisticaDeClientesMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctEstadisticaDeClientes = EstadisticaDeClientesMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on EstadisticaDeClientes's data fields
		$this->lstCliente = $this->mctEstadisticaDeClientes->lstCliente_Create();
		$this->calFechaPrimeraGuia = $this->mctEstadisticaDeClientes->calFechaPrimeraGuia_Create();
		$this->calFechaUltimaGuia = $this->mctEstadisticaDeClientes->calFechaUltimaGuia_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctEstadisticaDeClientes->EstadisticaDeClientes && !isset($_SESSION['DataEstadisticaDeClientes'])) {
            $_SESSION['DataEstadisticaDeClientes'] = serialize(array($this->mctEstadisticaDeClientes->EstadisticaDeClientes));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataEstadisticaDeClientes']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctEstadisticaDeClientes->EstadisticaDeClientes->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'EstadisticaDeClientes';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctEstadisticaDeClientes->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('EstadisticaDeClientes'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctEstadisticaDeClientes->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('EstadisticaDeClientes',$this->mctEstadisticaDeClientes->EstadisticaDeClientes->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctEstadisticaDeClientes->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctEstadisticaDeClientes->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/estadistica_de_clientes_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctEstadisticaDeClientes->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the EstadisticaDeClientesMetaControl
		$this->mctEstadisticaDeClientes->SaveEstadisticaDeClientes();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctEstadisticaDeClientes->TablasRelacionadasEstadisticaDeClientes();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lstCliente->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the EstadisticaDeClientesMetaControl
			$this->mctEstadisticaDeClientes->DeleteEstadisticaDeClientes();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctEstadisticaDeClientes->EstadisticaDeClientes->Id;
        $_SESSION['TablRefe'] = 'EstadisticaDeClientes';
        $_SESSION['RegiReto'] = 'estadistica_de_clientes_edit.php/'.$this->mctEstadisticaDeClientes->EstadisticaDeClientes->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/estadistica_de_clientes_list.php');
    }

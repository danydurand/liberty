 ...
 * of the PendientesPuntoVirtual class.  It uses the code-generated
 * PendientesPuntoVirtualMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a PendientesPuntoVirtual columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both pendientes_punto_virtual_edit.php AND
 * pendientes_punto_virtual_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class PendientesPuntoVirtualEditFormBase extends QForm {
	// Local instance of the PendientesPuntoVirtualMetaControl
	/**
	 * @var PendientesPuntoVirtualMetaControlGen mctPendientesPuntoVirtual
	 */
	protected $mctPendientesPuntoVirtual;
	...
	// Controls for PendientesPuntoVirtual's Data Fields
	protected $lblId;
	protected $txtNumeReci;
	protected $calFechPago;
	protected $txtMontMovi;
	protected $txtCeduClie;
	protected $txtCodiLibx;
	protected $txtDescStat;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the PendientesPuntoVirtualMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctPendientesPuntoVirtual = PendientesPuntoVirtualMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on PendientesPuntoVirtual's data fields
		$this->lblId = $this->mctPendientesPuntoVirtual->lblId_Create();
		$this->txtNumeReci = $this->mctPendientesPuntoVirtual->txtNumeReci_Create();
		$this->calFechPago = $this->mctPendientesPuntoVirtual->calFechPago_Create();
		$this->txtMontMovi = $this->mctPendientesPuntoVirtual->txtMontMovi_Create();
		$this->txtCeduClie = $this->mctPendientesPuntoVirtual->txtCeduClie_Create();
		$this->txtCodiLibx = $this->mctPendientesPuntoVirtual->txtCodiLibx_Create();
		$this->txtDescStat = $this->mctPendientesPuntoVirtual->txtDescStat_Create();
		$this->txtUsuaRegi = $this->mctPendientesPuntoVirtual->txtUsuaRegi_Create();
		$this->calFechRegi = $this->mctPendientesPuntoVirtual->calFechRegi_Create();
		$this->txtHoraRegi = $this->mctPendientesPuntoVirtual->txtHoraRegi_Create();
		$this->txtCodiCkpt = $this->mctPendientesPuntoVirtual->txtCodiCkpt_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctPendientesPuntoVirtual->PendientesPuntoVirtual && !isset($_SESSION['DataPendientesPuntoVirtual'])) {
            $_SESSION['DataPendientesPuntoVirtual'] = serialize(array($this->mctPendientesPuntoVirtual->PendientesPuntoVirtual));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataPendientesPuntoVirtual']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctPendientesPuntoVirtual->PendientesPuntoVirtual->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'PendientesPuntoVirtual';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctPendientesPuntoVirtual->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('PendientesPuntoVirtual'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctPendientesPuntoVirtual->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('PendientesPuntoVirtual',$this->mctPendientesPuntoVirtual->PendientesPuntoVirtual->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctPendientesPuntoVirtual->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctPendientesPuntoVirtual->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/pendientes_punto_virtual_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctPendientesPuntoVirtual->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the PendientesPuntoVirtualMetaControl
		$this->mctPendientesPuntoVirtual->SavePendientesPuntoVirtual();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctPendientesPuntoVirtual->TablasRelacionadasPendientesPuntoVirtual();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the PendientesPuntoVirtualMetaControl
			$this->mctPendientesPuntoVirtual->DeletePendientesPuntoVirtual();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctPendientesPuntoVirtual->PendientesPuntoVirtual->Id;
        $_SESSION['TablRefe'] = 'PendientesPuntoVirtual';
        $_SESSION['RegiReto'] = 'pendientes_punto_virtual_edit.php/'.$this->mctPendientesPuntoVirtual->PendientesPuntoVirtual->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/pendientes_punto_virtual_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/pendientes_punto_virtual_edit.php');
    }

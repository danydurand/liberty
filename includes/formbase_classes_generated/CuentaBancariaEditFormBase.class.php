 ...
 * CuentaBancariaMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a CuentaBancaria columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both cuenta_bancaria_edit.php AND
 * cuenta_bancaria_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class CuentaBancariaEditFormBase extends QForm {
	// Local instance of the CuentaBancariaMetaControl
	/**
	 * @var CuentaBancariaMetaControlGen mctCuentaBancaria
	 */
	protected $mctCuentaBancaria;
	...
	// Controls for CuentaBancaria's Data Fields
	protected $lblId;
	protected $lstBanco;
	protected $txtCodigo;
	protected $txtNumero;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the CuentaBancariaMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctCuentaBancaria = CuentaBancariaMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on CuentaBancaria's data fields
		$this->lblId = $this->mctCuentaBancaria->lblId_Create();
		$this->lstBanco = $this->mctCuentaBancaria->lstBanco_Create();
		$this->txtCodigo = $this->mctCuentaBancaria->txtCodigo_Create();
		$this->txtNumero = $this->mctCuentaBancaria->txtNumero_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctCuentaBancaria->CuentaBancaria && !isset($_SESSION['DataCuentaBancaria'])) {
            $_SESSION['DataCuentaBancaria'] = serialize(array($this->mctCuentaBancaria->CuentaBancaria));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataCuentaBancaria']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctCuentaBancaria->CuentaBancaria->Id) {
	...
	protected function lblTituForm_Create() {
        ...
        $this->lblTituForm->Text = 'CuentaBancaria';
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctCuentaBancaria->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('CuentaBancaria'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctCuentaBancaria->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('CuentaBancaria',$this->mctCuentaBancaria->CuentaBancaria->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctCuentaBancaria->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctCuentaBancaria->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objCuentaBancaria = CuentaBancaria::LoadByCodigo($this->txtCodigo->Text)) && ($objCuentaBancaria->Id != $this->mctCuentaBancaria->CuentaBancaria->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/cuenta_bancaria_edit.php/'.$objRegiTabl->Id);
    }
    ...
    protected function verificarNavegacion() {
        if ($this->mctCuentaBancaria->EditMode) {
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the CuentaBancariaMetaControl
		$this->mctCuentaBancaria->SaveCuentaBancaria();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctCuentaBancaria->TablasRelacionadasCuentaBancaria();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the CuentaBancariaMetaControl
			$this->mctCuentaBancaria->DeleteCuentaBancaria();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctCuentaBancaria->CuentaBancaria->Id;
        $_SESSION['TablRefe'] = 'CuentaBancaria';
        $_SESSION['RegiReto'] = 'cuenta_bancaria_edit.php/'.$this->mctCuentaBancaria->CuentaBancaria->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/cuenta_bancaria_list.php');
    }

 ...
 */
abstract class CuponEditFormBase extends QForm {
	// Local instance of the CuponMetaControl
	/**
	 * @var CuponMetaControlGen mctCupon
	 */
	protected $mctCupon;
	...
	// Controls for Cupon's Data Fields
	protected $lblId;
	protected $txtNumero;
	protected $txtPorcentajeDescuento;
	protected $txtCargadoPor;
	protected $calCargadoEl;
	protected $txtUsadoPor;
	protected $calUsadoEl;
	protected $txtGuiaId;
	protected $txtMontoDescuento;
	protected $txtReceptoria;
	protected $txtTipo;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the CuponMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctCupon = CuponMetaControl::CreateFromPathInfo($this);
		...
		$this->lblId = $this->mctCupon->lblId_Create();
		$this->txtNumero = $this->mctCupon->txtNumero_Create();
		$this->txtPorcentajeDescuento = $this->mctCupon->txtPorcentajeDescuento_Create();
		$this->txtCargadoPor = $this->mctCupon->txtCargadoPor_Create();
		$this->calCargadoEl = $this->mctCupon->calCargadoEl_Create();
		$this->txtUsadoPor = $this->mctCupon->txtUsadoPor_Create();
		$this->calUsadoEl = $this->mctCupon->calUsadoEl_Create();
		$this->txtGuiaId = $this->mctCupon->txtGuiaId_Create();
		$this->txtMontoDescuento = $this->mctCupon->txtMontoDescuento_Create();
		$this->txtReceptoria = $this->mctCupon->txtReceptoria_Create();
		$this->txtTipo = $this->mctCupon->txtTipo_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctCupon->Cupon && !isset($_SESSION['DataCupon'])) {
            $_SESSION['DataCupon'] = serialize(array($this->mctCupon->Cupon));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataCupon']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctCupon->Cupon->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctCupon->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Cupon'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctCupon->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Cupon',$this->mctCupon->Cupon->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctCupon->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctCupon->EditMode;
    }
...
/**
	...
	protected function Form_Validate() {
		...
		// Check for records that may violate Unique Clauses
			if (($objCupon = Cupon::LoadByGuiaId($this->txtGuiaId->Text)) && ($objCupon->Id != $this->mctCupon->Cupon->Id )){
				$blnToReturn = false;
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/cupon_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the CuponMetaControl
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctCupon->TablasRelacionadasCupon();
		...
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the CuponMetaControl
			$this->mctCupon->DeleteCupon();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctCupon->Cupon->Id;
        $_SESSION['TablRefe'] = 'Cupon';
        $_SESSION['RegiReto'] = 'cupon_edit.php/'.$this->mctCupon->Cupon->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/cupon_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/cupon_edit.php');
    }

 ...
 * of the Reporte class.  It uses the code-generated
 * ReporteMetaControl class, which has meta-methods to help with
 * easily creating/defining controls to modify the fields of a Reporte columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 * 
 * NOTE: This file is overwritten on any code regenerations.  If you want to make
 * permanent changes, it is STRONGLY RECOMMENDED to move both reporte_edit.php AND
 * reporte_edit.tpl.php out of this Form Drafts directory.
 *
 * @package My QCubed Application
 * @subpackage FormBaseObjects
 */
abstract class ReporteEditFormBase extends QForm {
	// Local instance of the ReporteMetaControl
	/**
	 * @var ReporteMetaControlGen mctReporte
	 */
	protected $mctReporte;
	...
	// Controls for Reporte's Data Fields
	protected $lblClavRepo;
	protected $txtNumeSesi;
	protected $txtLineDato;
	...
	protected function Form_Create() {
        ...
        // Use the CreateFromPathInfo shortcut (this can also be done manually using the ReporteMetaControl constructor)
        // MAKE SURE we specify "$this" as the MetaControl's (and thus all subsequent controls') parent
        $this->mctReporte = ReporteMetaControl::CreateFromPathInfo($this);
		...
		// Call MetaControl's methods to create qcontrols based on Reporte's data fields
		$this->lblClavRepo = $this->mctReporte->lblClavRepo_Create();
		$this->txtNumeSesi = $this->mctReporte->txtNumeSesi_Create();
		$this->txtLineDato = $this->mctReporte->txtLineDato_Create();
    ...
    protected function determinarPosicion() {
        if ($this->mctReporte->Reporte && !isset($_SESSION['DataReporte'])) {
            $_SESSION['DataReporte'] = serialize(array($this->mctReporte->Reporte));
        }
        $this->arrDataTabl = unserialize($_SESSION['DataReporte']);
        ...
        foreach ($this->arrDataTabl as $objTable) {
            if ($objTable->Id == $this->mctReporte->Reporte->Id) {
    ...
    protected function btnNuevRegi_Create() {
        ...
        $this->btnNuevRegi->Visible = $this->mctReporte->EditMode;
    }
	...
	protected function btnDelete_Create() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Reporte'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->mctReporte->EditMode;
	}
    ...
    protected function btnLogxCamb_Create() {
        ...
        $this->btnLogxCamb->Visible = Log::CountByTablaRef('Reporte',$this->mctReporte->Reporte->Id);
    }
    ...
    protected function btnNuevSmal_Create() {
        ...
        $this->btnNuevSmal->Visible = $this->mctReporte->EditMode;
    }
    ...
    protected function btnBorrSmal_Create() {
        ...
        $this->btnBorrSmal->Visible = $this->mctReporte->EditMode;
    }
...
/**
    ...
    protected function btnProxRegi_Click() {
        ...
        QApplication::Redirect(__SIST__.'/reporte_edit.php/'.$objRegiTabl->Id);
    }
	...
	protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
		// Delegate "Save" processing to the ReporteMetaControl
		$this->mctReporte->SaveReporte();
	...
	protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
		...
		//----------------------------------------
		$blnTodoOkey = true;
		$arrTablRela = $this->mctReporte->TablasRelacionadasReporte();
		if (count($arrTablRela)) {
			$strTablRela = implode(',',$arrTablRela);
				
			//$this->lblClavRepo->Warning = sprintf('Existen registros relacionados en %s',$strTablRela);
            $this->mensaje(sprintf('Existen registros relacionados en %s',$strTablRela),'m','d','hand-stop-o');
			$blnTodoOkey = false;
		}
		if ($blnTodoOkey) {
			// Delegate "Delete" processing to the ReporteMetaControl
			$this->mctReporte->DeleteReporte();
			$this->RedirectToListPage();
		}
	}

    protected function btnLogxCamb_Click() {
        $_SESSION['RegiRefe'] = $this->mctReporte->Reporte->Id;
        $_SESSION['TablRefe'] = 'Reporte';
        $_SESSION['RegiReto'] = 'reporte_edit.php/'.$this->mctReporte->Reporte->Id;
        QApplication::Redirect(__SIST__.'/log_list.php');
    }

    protected function btnVolvList_Click() {
        QApplication::Redirect(__SIST__.'/reporte_list.php');
    }

    protected function btnNuevRegi_Click() {
        QApplication::Redirect(__SIST__.'/reporte_edit.php');
    }

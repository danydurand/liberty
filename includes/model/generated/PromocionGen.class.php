<?php
	 ...
	 * @property-read Guia $_Guia the value for the private _objGuia (Read-Only) if set due to an expansion on the guia.promocion_id reverse relationship
	 * @property-read Guia[] $_GuiaArray the value for the private _objGuiaArray (Read-Only) if set due to an ExpandAsArray on the guia.promocion_id reverse relationship
	...
	class PromocionGen extends QBaseClass implements IteratorAggregate {
		 ...
		 * (of type Guia), if this Promocion object was restored with
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case '_Guia':
					 ...
					 * if set due to an expansion on the guia.promocion_id reverse relationship
					 * @return Guia
					 */
				...
				case '_GuiaArray':
					 ...
					 * if set due to an ExpandAsArray on the guia.promocion_id reverse relationship
					 * @return Guia[]
					 */
		...
		*/
		public function GetGuiaArray($objOptionalClauses = null) {
			...
			try {
				return Guia::LoadArrayByPromocionId($this->intId, $objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		*/
		public function CountGuias() {
			...
			return Guia::CountByPromocionId($this->intId);
		}
		...
		*/
		public function AssociateGuia(Guia $objGuia) {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Unable to call AssociateGuia on this unsaved Promocion.');
			if ((is_null($objGuia->NumeGuia)))
				throw new QUndefinedPrimaryKeyException('Unable to call AssociateGuia on this Promocion with an unsaved Guia.');
			...
			$objDatabase->NonQuery('
				UPDATE
					`guia`
				SET
					`promocion_id` = ' . $objDatabase->SqlVariable($this->intId) . '
				WHERE
		...
		*/
		public function UnassociateGuia(Guia $objGuia) {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateGuia on this unsaved Promocion.');
			if ((is_null($objGuia->NumeGuia)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateGuia on this Promocion with an unsaved Guia.');
	...
	class QQNodePromocion extends QQNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'Guia':
					return new QQReverseReferenceNodeGuia($this, 'guia', 'reverse_reference', 'promocion_id', 'Guia');

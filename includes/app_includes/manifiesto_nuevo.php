...
class PDF extends FPDF
{
	...
	protected $mensaje;
	protected $LineasPag;
	protected $intCuentaItems;


	function setVariables($enca,$just,$anch,$vert,$manifiesto='',$fechMani='',$mensa, $lineas=0) {
		$this->enca = $enca;
		$this->just = $just;
		$this->anch = $anch;
		$this->vert = $vert;
	...
	function Header() {
		...
		$this->SetXY(243,8);
//		$this->Cell(3,1,date('d/m/Y h:i'),0,1,'L');
		$this->Cell(3,1,$this->fecha,0,1,'L');
		$this->Cell(1);
		//		$this->Cell(6,9,$this->direccion);
		//		$this->SetX($posi_imag+16);
		$this->SetXY(120,3);
		$this->Cell(6,20,'Manifiesto Nro '.$this->manifiesto,0,0);
		$this->SetXY(14,14);
		$this->SetFont('Arial','',8);
		$this->MultiCell(260,10,$arrDeclara[0],0);
		$this->SetXY(14,15);
		if (isset($arrDeclara[1])) {
			$this->MultiCell(200,15,$arrDeclara[1],0);
		}
		$this->SetXY(14,18);
		if (isset($arrDeclara[2])) {
			$this->MultiCell(200,15,$arrDeclara[2],0);
		}
		//		$this->SetXY(6,19);
	...
	function Footer()
	{
		...
		$y=$this->GetY();
		$this->SetXY($x+5,$y+2);
		$this->Cell(30,4,'Elaborado por:',0,1);
		$this->SetXY($x+5,$y+8);
	...
	function CheckPageBreak($h)
	{
		...
		if($this->GetY()+$h>$this->PageBreakTrigger) {
			...
			if ( ($this->LineasPag > 0 ) && ($this->intCuentaItems >= $this->LineasPag)) {
				$this->AddPage($this->CurOrientation);
			}
		}
	}

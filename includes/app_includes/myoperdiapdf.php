...
class PDF extends FPDF
{
	...
	function setVariables($enca,$just,$anch,$vert,$manifiesto='',$tipodocu='',$mensa)
	{
		$this->enca = $enca;
		$this->just = $just;
		$this->anch = $anch;
	...
	function Header()
	{
		...
		$this->Cell(1);
		$this->Cell(6,9,$this->mensaje);
		//		$this->SetX($posi_imag+16);
		$this->SetXY(120,1);
		$this->Cell(6,20,'Fecha impresion : '.date('d/m/Y'),0,0);
		$this->SetXY(50,3);
		$this->SetFont('Arial','',8);
//		$this->SetXY(8,20);
      $arrfech = explode('-',$this->tipoDocu);
//		$this->Cell(4,20,'Fecha de la Operacion : '.$this->tipoDocu);
		$this->Cell(4,20,'Fecha de la Operacion : '.$arrfech[2].'/'.$arrfech[1].'/'.$arrfech[0]);
...
//			$this->Cell($w[$i],7,$this->enca[$i],1,0,$this->just[$i],1);
			$this->Cell($w[$i],7,$this->enca[$i],1,0,'C',1);
		}
		//Salto de línea
		$this->Ln(7);
	}
...
//			if (($blnFlag) && ($intpos >= $i))  {
//				$this->Rect($x,$y,$w,$h,' ');	
//				$this->MultiCell($w,6,$data[$i],0,$a,1);
//			} else {
//				$this->Rect($x,$y,$w,$h);

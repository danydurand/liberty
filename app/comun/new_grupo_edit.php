...
class NewGrupoEditForm extends NewGrupoEditFormBase {
    ...
    protected function btnDelete_Click($strFormId, $strControlId, $strParameter) {
	    if ($this->mctNewGrupo->NewGrupo->CountUsuariosAsGrupo() > 0) {
	        $strTextMens = 'Existen Usuarios asociados a este Grupo.  No se puede eliminar';
	        $this->mensaje($strTextMens,'','d',__iHAND__);
	        return;
        }
        //-------------------------------------------------
        // Se eliminan los permisos relacionados al grupo
        //-------------------------------------------------
        $arrPermGrup = Permiso::LoadArrayByGrupoId($this->mctNewGrupo->NewGrupo->Id);
        foreach ($arrPermGrup as $objPermGrup) {
            $objPermGrup->Delete();
        }
        //--------------------------------------
        // Se buscan otras tablas relacionadas
        //--------------------------------------
        $arrTablRela = $this->mctNewGrupo->TablasRelacionadasNewGrupo();
        if (count($arrTablRela)) {
            $strTablRela = implode(',',$arrTablRela);
            $strTextMens = sprintf('Existen registros relacionados en <b>%s</b>',$strTablRela);
            $this->mensaje($strTextMens,'','d',__iHAND__);
            return;
        }
    ...
    protected function RedirectToListPage() {
        ...
        QApplication::Redirect(__COM__."/new_grupo_list.php");
    }

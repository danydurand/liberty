...
class ImprimirManifiestoHe extends FormularioBaseKaizen {
	...
	protected function Form_Create() {
		parent::Form_Create();
		$this->lblTituForm->Text = QApplication::Translate('Imprimir Reporte');
	...
	protected function lstOperAbie_Change() {
		...
		if (!is_null($this->lstOperAbie->SelectedValue)) {
			...
			$dttFechMani   = SumaRestaDiasAFecha($dttFechDhoy,90,"-");
	...
	protected function lstTipoOper_Change() {
		...
		if (!is_null($this->lstTipoOper->SelectedValue)) {
         ...
         $objClausula[] = QQ::OrderBy(QQN::SdeOperacion()->CodiRuta);
			foreach (SdeOperacion::LoadArrayByCodiTipo($this->lstTipoOper->SelectedValue,$objClausula) as $objOperacion) {
				//---------------------------------------------------------------------
